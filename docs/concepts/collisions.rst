...
- **Gap** (``shape_gap``): Surface gap (outward offset) used in contact generation (see :ref:`Shape Configuration`).
...
.. list-table::
   ...
   * - Parameter
     - Description
   * - ``gap``
     - Surface gap (outward offset). Pairwise: summed (``gap_a + gap_b``). Creates visible gap at rest. Essential for thin shells and cloth to improve simulation stability and reduce self-intersections. Default: 0.0.
   * - ``margin``
     - AABB expansion for early contact detection. Pairwise: summed. The margin only affects contact generation; effective rest distance is not affected and is only governed by ``gap``. Increasing the margin can help avoid tunneling of fast-moving objects because contacts are detected at a greater distance between objects. Must be such that ``gap + margin > 0``. Default: None (uses ``builder.rigid_contact_margin``, which defaults to 0.1).
   * - ``is_solid``
...
.. note::
   **Contact generation**: A contact is created when ``d < max(margin_a, margin_b)``, where 
   ``d = surface_distance - (gap_a + gap_b)``. The solver enforces ``d >= 0``, 
   so objects at rest settle with surfaces separated by ``gap_a + gap_b``.
...
The builder's ``rigid_contact_margin`` (default 0.1) applies to shapes without explicit ``margin``. Alternatively, use ``builder.default_shape_cfg.margin``.
...
.. list-table::
   ...
   * - ``rigid_contact_gap0``, ``rigid_contact_gap1``
     - Shape gap at each contact point.
...
.. code-block:: usda
    ...
    def Xform "Box" (
        ...
        custom float newton:contact_gap = 0.00001
    }
...
- :attr:`~newton.Model.shape_margin` - Per-shape contact margins
- :attr:`~newton.Model.shape_gap` - Per-shape gap values
