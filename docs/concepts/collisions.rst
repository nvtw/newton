...
.. note::
   ...
   ``ShapeConfig.has_particle_collision``).

.. note::
   ``HFIELD`` support is currently for rigid collisions against convex shapes
   (sphere/capsule/box/cylinder/cone/ellipsoid/convex mesh). Unsupported rigid pairs
   include heightfield-heightfield, plane-heightfield, and mesh-heightfield.

Ellipsoid and ConvexMesh are supported; they are omitted from the compact matrix above for readability.
