...
- **Thickness** (``shape_margin``): Surface margin used in contact generation (see :ref:`Shape Configuration`).
...
.. list-table::
   ...
   * - Parameter
     - Description
   * - ``margin``
     - Surface margin. Pairwise: summed (``margin_a + margin_b``). Creates visible gap at rest. Essential for thin shells and cloth to improve simulation stability and reduce self-intersections. Default: 1e-5.
   * - ``gap``
     - Speculative contact window on top of margin. Pairwise: summed (``gap_a + gap_b``). Contacts are generated when separation is within ``(margin_a + margin_b) + (gap_a + gap_b)`` and become active/supporting once separation reaches ``margin_a + margin_b``. Default: None (uses ``builder.rigid_contact_gap``, which defaults to 0.1).
   * - ``is_solid``
...
.. note::
   **Contact generation**: Let ``d = surface_distance - (margin_a + margin_b)``.
   Contacts are generated when ``d <= gap_a + gap_b`` and are active/supporting when
   ``d <= 0``. The solver enforces ``d >= 0`` at rest, so objects settle with
   surfaces separated by ``margin_a + margin_b``.
...
The builder's ``rigid_contact_gap`` (default 0.1) applies to shapes without explicit ``gap``. Alternatively, use ``builder.default_shape_cfg.gap``.
...
.. code-block:: python
    ...
    pipeline = CollisionPipeline(model, soft_gap=0.01)
    contacts = pipeline.contacts()
    pipeline.collide(state, contacts)
...
.. list-table::
   ...
   * - ``rigid_gap0``, ``rigid_gap1``
     - Shape margin at each contact point.
...
- :attr:`~newton.Model.shape_gap` - Per-shape contact margins
- :attr:`~newton.Model.shape_margin` - Per-shape margin values
