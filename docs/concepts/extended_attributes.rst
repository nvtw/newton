...
- Solvers populate extended attributes they support. :class:`~newton.solvers.SolverMuJoCo` populates ``body_qdd``, ``body_parent_f``, ``mujoco:qfrc_actuator``, and ``force``. :class:`~newton.solvers.SolverXPBD` populates ``body_parent_f`` and ``force``; the reported wrenches are approximate (XPBD applies relaxation factors to each constraint correction and is not momentum-conserving), so they should be treated as the *applied* constraint reaction rather than an exact analytic value. For simple decoupled cases (e.g. a single dynamic body suspended from a kinematic or world parent) the values converge to within the integrator's first-order time-stepping bias.
