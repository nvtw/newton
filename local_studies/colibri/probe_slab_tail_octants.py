    ...
    for step in (744, 745, 746):
        ...
        for repeat in range(2):
            ...
            if name == "reduced_predictive" and step == 746 and repeat == 0:
                ...
                np.savez("/tmp/colibri_slab_tail_octants_746_reducer.npz",
            ...
            if repeat == 0:
                np.savez(
                    f"/tmp/colibri_slab_tail_octants_pair_{step}_{name}.npz",
                    point0=p0,
                    point1=p1,
                    normals=normals,
                    shapes=shapes,
...
with open("/tmp/colibri_slab_tail_octants_frozen_coverage.json", "w") as stream:
