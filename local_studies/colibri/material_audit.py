    ...
    if p.IsA(UsdShade.Material) and p.HasAPI(UsdPhysics.MaterialAPI):
        ...
        materials.append(
            dict(
                ...
                mu=api.GetDynamicFrictionAttr().Get(),
                mu_static=api.GetStaticFrictionAttr().Get(),
                restitution=api.GetRestitutionAttr().Get(),
                extra={
                    a.GetName(): str(a.Get())
                    for a in p.GetAttributes()
                    if a.HasAuthoredValueOpinion() and a.GetName().startswith("physxMaterial:")
                },
            )
        )
shapes = []
...
for name, kind, label, data, dimensions in values["SHAPES"]:
    ...
    expected = values["SHAPE_MATERIALS"].get(label, (1000.0, 0.5))
    actual = (1000.0, 0.5)
    ...
    record = dict(
        label=label,
        material=str(mat.GetPath()) if mat else None,
        relation=str(rel.GetPath()) if rel else None,
        source=actual,
        literal=expected,
        collision=p.GetAttribute("physics:collisionEnabled").Get(),
        approx=p.GetAttribute("physics:approximation").Get(),
        attrs={
            a.GetName(): str(a.Get())
            for a in p.GetAttributes()
            if a.HasAuthoredValueOpinion()
            and a.GetName().startswith(
                (
                    "physxCollision:",
    ...
    if abs(actual[0] - expected[0]) > max(1e-3, actual[0] * 1e-6) or actual[1] != expected[1]:
...
print(
    ...
    [
        (x["label"], x["attrs"])
        for x in shapes
        if x["attrs"] and any(not key.startswith("physxSDFMeshCollision:") for key in x["attrs"])
    ],
)
