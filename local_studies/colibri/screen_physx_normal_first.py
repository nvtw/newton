...
def main():
    ...
    try:
        ...
        poses = physx.create_tensor_binding(
            prim_paths=json.loads(Path("/tmp/colibri_usd_rigid_body_paths.json").read_text()), tensor_type=TensorType.RIGID_BODY_POSE, raise_if_empty=True
        )
        ...
        print("INITIAL_RUNTIME_VELOCITY",float(np.max(abs(velocity))),flush=True)
        properties = {}
        for name in ("MASS", "INERTIA", "COM_POSE", "CONTACT_OFFSET", "REST_OFFSET", "SHAPE_FRICTION_AND_RESTITUTION"):
            binding = physx.create_tensor_binding(prim_paths=poses.prim_paths, tensor_type=getattr(TensorType, "RIGID_BODY_" + name))
        ...
        report = {
            "solver": "PhysX", "package_version": ovphysx.__version__,
            "source": args.usd, "body_count": poses.count, "frames": args.frames,
            "warmup_frames": args.warmup, "outer_steps_per_frame": 2,
            "outer_dt": 1.0/120.0, "settings": "authored USD, including damping and scene iteration minimum",
        ...
        np.savez(
            ...
            initial=initial, q=position, qd=velocity, paths=poses.prim_paths, frame_times=times, **properties,
        )
