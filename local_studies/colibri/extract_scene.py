...
obj_vertices = {
    name: np.array(
        [list(map(float, line.split()[1:4])) for line in f.read_text().splitlines() if line.startswith("v ")]
    )
    for name, f in obj_files.items()
}
...
for p in stage.Traverse():
    ...
    if p.GetName().startswith("Cylinder"):
        ...
        shapes.append(
            (
                ...
                label,
                pose(cylinder_mat),
                (round(50 * radial_x * scale, 9), round(50 * axis.GetLength() * scale, 9)),
            )
        )
        continue
    ...
    if match:
        if match not in copied:
            ...
            lines = [
                "v " + " ".join(f"{float(x) * scale:.10g}" for x in line.split()[1:4])
...
collision_labels = [
    str(p.GetPath()).removeprefix(root)
    for p in stage.Traverse()
    if str(p.GetPath()).startswith(root)
...
print(
    ...
    len(shapes),
    "cylinders",
    sum(x[1] == "cylinder" for x in shapes),
    "matched OBJ files",
    len(copied),
    "USD fallback meshes",
    len(fallbacks),
