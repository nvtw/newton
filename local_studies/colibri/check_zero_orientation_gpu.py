...
def main():
    ...
    ids=np.flatnonzero(np.all(old[:,1].view(np.uint32)==old[:,3].view(np.uint32),axis=1)&
    ...
    wp.launch(repeat,dim=len(q),inputs=[qa,out,wp.float32(0.)],device=args.device)
    values=out.numpy()
    cycles=np.all(values[:,1].view(np.uint32)==values[:,3].view(np.uint32),axis=1)&np.any(values[:,1].view(np.uint32)!=values[:,2].view(np.uint32),axis=1)
    records=[]
    for omega in (0.,1e-8,1e-6,1e-3):
        outputs=[]
        for kernel in (_integrate_velocities_kernel,exact_zero_integrate):
            ...
            for _step in range(4):
                wp.launch(kernel,dim=len(q),inputs=[b,wp.float32(1./3600.)],device=args.device)
                states.append(b.orientation.numpy())
            outputs.append({"q": np.asarray(states),"v": b.velocity.numpy(),"w": b.angular_velocity.numpy(),
                                "x": b.position.numpy(),"inertia": b.inverse_inertia_world.numpy()})
        ...
        records.append({"omega": omega,"baseline_changed": int(np.sum(np.any(baseline["q"][0]!=q,axis=1))),
                            "candidate_changed": int(np.sum(np.any(candidate["q"][0]!=q,axis=1))),
                            "nonzero_bytes_equal": bool(omega!=0)})
    report={"device": args.device,"input_cpu_cycles": len(q),"device_helper_cycles": int(cycles.sum()),"cases": records,
    ...
    Path("/tmp/colibri_zero_orientation_"+args.device.replace(":","_")+".json").write_text(json.dumps(report,indent=2))
