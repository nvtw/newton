...
e=Example(ViewerNull(),argparse.Namespace(body_count=15,mode="maximal",layout="single_world",substeps=10,iterations=8,outer_substeps=2,source_contact_offsets=True,mesh_cylinders=True))
...
def traced_launch(kernel,*args,**kwargs):
    ...
    if warm: report("before-projected-warm")
    result=old_launch(kernel,*args,**kwargs)
    if warm: report("after-projected-warm")
    return result
