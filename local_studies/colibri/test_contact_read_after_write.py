...
import numpy as np
import warp as wp
from newton._src.solvers.phoenx.constraints.contact_container import ContactContainer, contact_container_zeros, cc_get_normal_lambda
...
def _probe(cc: ContactContainer, out: wp.array[wp.float32]):
    ...
    impulse = contact_project_normal_velocity_update(
        cc, 0, wp.vec3(1.0, 0.0, 0.0), wp.float32(0.0), wp.float32(0.1),
        wp.float32(-0.1), wp.float32(1.0), wp.float32(0.0), wp.float32(1.0),
        wp.float32(0.001), wp.float32(10.0), wp.float32(0.2),
    )
    out[0] = old
...
class TestReadAfterWrite(unittest.TestCase):
    def test_probe(self):
        ...
        np.testing.assert_allclose(out.numpy(), [0., .0002, .0002, .0002], atol=1e-10)
