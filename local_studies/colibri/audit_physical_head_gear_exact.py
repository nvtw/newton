...
for i in np.flatnonzero(witnesses["query_gap"] < -0.001):
    ...
    record = dict(
        id=int(i),
        post_stored_gap=float(witnesses["query_gap"][i]),
        source_shape=int(shapes[source]),
        target_shape=int(shapes[target]),
...
print(
    json.dumps(
        dict(
            count=len(rows),
            topology=topology,
            deepest=min(rows, key=lambda r: r["post_stored_gap"]),
            generation_distance_range=[
                min(r["generation"]["signed_distance"] for r in rows),
                max(r["generation"]["signed_distance"] for r in rows),
            ],
            max_source_error=max(r["source_surface_error"] for r in rows),
        ),
        indent=2,
    )
)
