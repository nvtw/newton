...
for name, reduction, cap in (
    ...
    ("reduced_envelope", True, 0.005),
):
    restore = install_conservative_mesh_candidates() if name == "reduced_envelope" else None
    ...
    for step in (744, 745, 746):
        ...
        for repeat in range(2):
            ...
            if repeat == 0:
                np.savez(
                    f"/tmp/colibri_slab_tail_envelope_pair_{step}_{name}.npz",
                    point0=p0,
                    point1=p1,
                    normals=normals,
                    shapes=shapes,
...
with open("/tmp/colibri_slab_tail_envelope_frozen_coverage.json", "w") as stream:
