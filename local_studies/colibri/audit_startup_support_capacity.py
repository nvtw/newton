...
def capture(path):
    a=np.load(path);h=a["headers"].view(np.int32);W=np.zeros((6,6))
    ...
    for c in range(int(a["column_count"][0])):
        ...
        for k in range(h[5,c],h[5,c]+h[6,c]):
            if a["derived"][3,k]>0:continue
            n=a["lambdas"][:3,k].astype(float);t=a["lambdas"][3:6,k].astype(float)
            ...
            ids.append(k);lams.extend(a["after_impulses"][:,k]);mus.append(a["headers"][3,c])
    J=np.vstack(rows);lam=np.array(lams,float)
    after=np.r_[a["after_velocity"][1],a["after_angular_velocity"][1]].astype(float)
    free=after-W@J.T@lam
    count=len(ids);rays=np.zeros((count*3,count*64))
    ...
    B=W@J.T@rays*1e-4;scale=np.maximum(np.linalg.norm(B,axis=1),abs(free))
    lp=linprog(np.ones(count*64),A_eq=B/scale[:,None],b_eq=-free/scale,bounds=(0,None),
        method="highs",options={"primal_feasibility_tolerance":1e-10,"dual_feasibility_tolerance":1e-10})
    result=dict(source=str(path),points=ids,actual_after_twist=after.tolist(),free_twist=free.tolist(),
        cone_inner_polygon_feasible=bool(lp.success),status=lp.message)
    if not lp.success:
        outer=rays.copy()
        ...
        Bout=base@outer
        target=-free/scale
        dual=linprog(np.zeros(6),A_ub=-Bout.T,b_ub=np.zeros(Bout.shape[1]),
                     A_eq=target[None,:],b_eq=[-1.],bounds=[(None,None)]*6,method="highs")
        ...
        if dual.success:
            ...
            result["interpretation"]="Nonnegative exact circle dual and negative target dot certify fixed-reaction infeasibility, not fully coupled live motion necessity."
    if lp.success:
        value=rays@lp.x*1e-4;v=free+W@J.T@value;t=value.reshape(-1,3)
        result.update(velocity_residual=float(max(abs(v))),normal_sum_Ns=float(sum(t[:,0])),
...
def main():
    ...
    q=a["q_history"][:,body];v=a["qd_history"][:,body];times=a["history_times"]
    dx=np.diff(np.vstack([a["initial_q"][body,:3],q[:,:3]]),axis=0)
    peak=np.argsort(np.linalg.norm(dx[:,:2],axis=1))[-8:][::-1]
    motion=dict(source="/tmp/colibri_total_normal330.npz",
        largest_frame_translations=[dict(time=float(times[i]),translation_m=dx[i].tolist(),twist=v[i].tolist()) for i in peak],
    ...
    out=dict(motion=motion,frozen_capacity=results,
