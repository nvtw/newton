...
def hook(**kwargs):
    ...
    if calls % 8 == 1:
        ...
        invm = w.bodies.inverse_mass.numpy()
        invi = inertia_sym6_unpack_np(w.bodies.inverse_inertia_world.numpy())
        ...
        b0 = r.contact_body0.numpy()[:n]
        b1 = r.contact_body1.numpy()[:n]
        for k in range(n):
        ...
        mat = d.matrix.numpy()
        ar = np.zeros_like(a)
        rr = d.matrix_row.numpy()
        cl = d.matrix_column.numpy()
        st = d.matrix_storage.numpy()
        ar[rr, cl] = mat[st]
        ar[cl, rr] = mat[st]
        ...
        exact = (
            un - np.sum(rhs * np.linalg.pinv(a, rcond=1e-11) @ rhs, axis=0)
            if False
            else un - np.sum(rhs * (np.linalg.pinv(a, rcond=1e-11) @ rhs), axis=0)
        )
        ...
        np.savez(
            "/tmp/colibri_schur_ref_%d.npz" % calls,
            j=j,
            m=m,
            c=c,
            a=a,
