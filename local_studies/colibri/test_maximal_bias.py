...
j=b.add_joint_revolute(-1,body,axis=wp.normalize(wp.vec3(0,0.01,1)))
b.add_articulation([j])
b.add_ground_plane()
m=b.finalize(device="cuda:0")
p=newton.CollisionPipeline(m,rigid_contact_max=32,contact_matching="sticky")
s=newton.solvers.SolverPhoenX(m,collision_pipeline=p,articulation_mode="maximal",step_layout="single_world",sor_boost=1.0,substeps=1,solver_iterations=8)
