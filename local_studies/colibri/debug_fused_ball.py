...
class TestFreeBallMomentum(unittest.TestCase):
    def test_ball_anchor_and_internal_motor(self):
        ...
        print("INITIAL",state.body_q.numpy(),state.body_qd.numpy(),flush=True)
        print("MAP",solver._joint_constraints.joint_idx_to_cid.numpy(),solver.world._joint_pgs_enabled.numpy(),flush=True)
        with wp.ScopedCapture(model.device) as capture:
            solver.step(state,state,control,None,.001)
        for _ in range(200):
            wp.capture_launch(capture.graph)
            if _ in (0,1,9,199):
                d=solver._direct_equality_system
                print("STEP",_,state.body_q.numpy(),state.body_qd.numpy(),flush=True)
                print("BLOCK",solver.world.constraints.bilateral.row_count.numpy(),
                      solver.world.constraints.bilateral.valid.numpy(),solver.world._num_active_constraints.numpy(),flush=True)
                print("BIAS",d.row_bias.numpy(),flush=True)

        np.testing.assert_allclose(_total_momentum(model,state),before,rtol=0,atol=2e-4)
        q=state.body_q.numpy()
        p0=wp.transform_point(wp.transform(q[0,:3],q[0,3:]),wp.vec3(.3,0,0))
        p1=wp.transform_point(wp.transform(q[1,:3],q[1,3:]),wp.vec3(-.2,0,0))
        ...
        for value in namespace.values():
            if isinstance(value, type) and issubclass(value, unittest.TestCase):
                for test in unittest.defaultTestLoader.loadTestsFromTestCase(value):
                    if "mass_splitting" not in test.id() and "multi_world" not in test.id() and "coupled_three_joint" not in test.id():
                        suite.addTest(test)
...
if __name__ == "__main__":
    ...
    with patch.object(newton.solvers.SolverPhoenX, "__init__", initialize):
        result = unittest.TextTestRunner().run(unittest.defaultTestLoader.loadTestsFromTestCase(TestFreeBallMomentum))
