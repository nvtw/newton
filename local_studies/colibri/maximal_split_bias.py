...
def report(label):
    ...
    impulse = cc.impulses.numpy()[:, :n]
    mobi = w._maximal_contact_schedule.mobility.numpy()[:, :n]
    if label.startswith("after-contact"):
        ...
        print(
            ...
            np.max(abs(v)),
            "impulses",
            [(int(k), impulse[:, k].tolist(), mobi[:, k].tolist(), float(cc.derived.numpy()[15, k])) for k in order],
            flush=True,
        )
