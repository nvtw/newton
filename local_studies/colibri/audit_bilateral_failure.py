...
print(
    ...
    json.dumps(
        [
            dict(
                joint=e.model.joint_label[rjoint[i]],
                local=int(local[i]),
                velocity=float(jv[i]),
                error=float(error[i]),
                bias=float(bias[i]),
...
for side in (0, 1):
    ...
    for sh, point in zip(shape, points_local):
        body = shape_body[sh]
        pts.append(
            np.array(wp.transform_point(wp.transform(q[body, :3], q[body, 3:]), wp.vec3(point))) if body >= 0 else point
        )
...
def row(i):
    return dict(
        shape0=e.model.shape_label[shape0[i]],
        shape1=e.model.shape_label[shape1[i]],
        gap=float(gaps[i]),
        velocity=float(vn[i]),
