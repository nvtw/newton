...
source=source.replace("np.testing.assert_allclose(after, before, rtol=0.0, atol=2.0e-4)","print('MOMENTUM', after-before, state.joint_q.numpy(), flush=True)")
...
source=source.replace("solver_iterations=8, sor_boost=1.0", "solver_iterations=8, velocity_iterations=0, sor_boost=1.0")
for mu, force, bend in [(0.0,0.0,True),(0.5,0.1,True)]:
    print("CASE",mu,force,bend,flush=True)
    s=source.replace("fill_(0.5)",f"fill_({mu})").replace("joint_force[-1] = 0.1",f"joint_force[-1] = {force}")
    if not bend:s=s.replace("[0.3, -0.6]","[0.0, 0.0]")
