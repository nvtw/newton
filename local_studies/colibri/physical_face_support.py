...
def seed_sweeps(a, seed, sweeps):
    ...
    for _ in range(sweeps):
        for p in range(len(a["mu"])):
            ...
            lam[row] = max(
                0.0, lam[row] - (gradient[row] + a["gamma"][p] * lam[row]) / (a["A"][row, row] + a["gamma"][p])
            )
            ...
            if np.linalg.norm(tangent) > radius:
                ...
                upper = max(float(np.linalg.norm(matrix)), 1.0)
                for _ in range(60):
                    if error(upper) <= 0:
                        break
                    upper *= 2
                shift = brentq(error, 0.0, upper, xtol=1e-12)
...
def solve_face(a, f, seed, active, stick):
    ...
    def evaluate(x):
        ...
        for p in np.flatnonzero(active):
            ...
            if stick[p]:
                ...
                for j in (1, 2):
                    eq = np.zeros(cursor)
                    eq[:dimension] = f[row + j]
                    equations.append(gradient[p, j])
                    jacobian.append(eq)
            elif a["mu"][p] > 0:
        ...
        top = -f.T @ derivative
        top[:, :dimension] += np.eye(dimension)
    ...
    for iteration in range(12):
        ...
        if cursor > 32:
            return lam, dict(
                accepted_face=False,
                reason="More than32unknowns; stop before dense redundant dual solve",
                unknowns=cursor,
            )
        try:
            ...
            return lam, dict(
                accepted_face=False, reason="Singular face Jacobian; no physical rank truncation", unknowns=cursor
            )
    ...
    residual, jac, lam = evaluate(x)
    return lam, dict(
        accepted_face=bool(np.max(abs(residual)) < 1e-10),
        face_error=float(np.max(abs(residual))),
        unknowns=cursor,
        history=history,
...
def audit(a, lam):
    ...
    report = dict(
        natural_error=float(np.max(abs(evaluate(lam)[0]))),
        minimum_normal=float(triples[:, 0].min()),
        minimum_normal_gradient=float(normal.min()),
        normal_complementarity=float(np.max(abs(normal * triples[:, 0]))),
        ...
        impulse_work=float(0.5 * (velocity + a["free"]) @ impulse),
        kinetic_change=float(
            0.5 * (velocity @ np.linalg.solve(a["W"], velocity) - a["free"] @ np.linalg.solve(a["W"], a["free"]))
        ),
    )
    report["accepted"] = bool(
        report["natural_error"] < 1e-8 and report["minimum_normal"] >= -1e-10 and report["cone_violation"] < 1e-8
    )
    return report
...
def main():
    ...
    for phase in ("biased", "relax"):
        ...
        for label, seed in (
            ("native", a["old"]),
            ("two_sweep", seed_sweeps(a, a["old"], 2)),
            ("reference_face_only", reference),
        ):
            ...
            try:
                ...
                report = dict(accepted_face=False, reason=str(exc))
                checked = {"accepted": False}
            record = dict(
                phase=phase,
                seed=label,
                active=np.flatnonzero(active).tolist(),
                sticking=np.flatnonzero(stick).tolist(),
                solve=report,
                audit=checked,
                oracle_face=label == "reference_face_only",
            )
