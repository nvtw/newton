...
from local_studies.colibri.cached_bilateral_rows import install
from newton._src.solvers.phoenx.tests import test_inactive_joint_prepare as transitions


class TestCachedBilateralRows(unittest.TestCase):
        ...
        try:
            for prismatic in (False, True):
                ...
                try:
                    ...
                    self.assertGreater(restore.stats["prepare"], 0)
                    self.assertGreater(restore.stats["sweep"], 0)
                    self.assertGreater(restore.stats["bind"], 0)
                finally:
                    restore()
