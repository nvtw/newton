...
@wp.kernel
def contact_gap(q:wp.array[wp.transform], shape_body:wp.array[wp.int32],
    s0:wp.array[wp.int32],s1:wp.array[wp.int32],p0:wp.array[wp.vec3],p1:wp.array[wp.vec3],
    normal:wp.array[wp.vec3],m0:wp.array[wp.float32],m1:wp.array[wp.float32], gap:wp.array[wp.float32]):
    i=wp.tid()
    b0=shape_body[s0[i]]
    b1=shape_body[s1[i]]
    x0=p0[i]
...
a=p.parse_args()
e=Example(ViewerNull(),argparse.Namespace(body_count=a.body_count,mode="reduced",layout="single_world",
    substeps=10,outer_substeps=2,iterations=8,source_contact_offsets=True,contact_gap=.001,mesh_cylinders=True,no_graph=a.forest))
if a.forest:
    from newton._src.solvers.phoenx.articulations.reduced_forest import ReducedForestContactSystem
    e.solver._reduced_articulation.forest_contact_system=ReducedForestContactSystem(e.solver._reduced_articulation,e.contact_capacity)
...
for frame in range(a.frames+1):
    ...
    wp.launch(contact_gap,dim=n,inputs=[e.state_0.body_q,e.model.shape_body,fresh.rigid_contact_shape0,
        fresh.rigid_contact_shape1,fresh.rigid_contact_point0,fresh.rigid_contact_point1,
        fresh.rigid_contact_normal,fresh.rigid_contact_margin0,fresh.rigid_contact_margin1],outputs=[gap],device=e.model.device)
    gaps=gap.numpy()[:n]
    ...
    if frame%10==0 or gaps[i]<-.003:
        print("FRAME",frame,"N",n,"MIN_GAP",float(gaps[i]),"PAIR",e.model.shape_label[s0[i]],e.model.shape_label[s1[i]],"SPEED",np.max(np.linalg.norm(e.state_0.body_qd.numpy(),axis=1)),flush=True)
    try:
        e.test_post_step()
