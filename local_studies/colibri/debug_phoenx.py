...
for frame in range(20):
    ...
    if not np.isfinite(q).all() or not np.isfinite(v).all() or np.nanmax(abs(v))>100:
        ...
        print("REACTIONS",[(e.model.joint_label[int(j)],float(x)) for j,x in zip(direct.topology.row_joint,direct.accumulated_impulse.numpy()) if abs(x)>10],flush=True)
        response=e.solver._direct_contact_response.data
        cc=e.solver.world._contact_container
        impulses=cc.impulses.numpy()[0]; derived=cc.derived.numpy(); mobility=response.mobility.numpy(); bodies0=response.contact_body0.numpy(); bodies1=response.contact_body1.numpy()
        for k in np.argsort(impulses)[-10:]:
            print("CONTACT",int(k),"impulse",float(impulses[k]),"bodies",e.model.body_label[bodies0[k]-1] if bodies0[k]>0 else "world",e.model.body_label[bodies1[k]-1] if bodies1[k]>0 else "world","mobility",mobility[:,k],"derived",derived[:,k],flush=True)
        ...
        for obj in [e.solver._direct_equality_system,getattr(e.solver._direct_contact_response,"data",None)]:
            ...
            for name,arr in vars(obj).items():
                if isinstance(arr,wp.array) and arr.dtype==wp.float32:
                    a=arr.numpy(); print(type(obj).__name__,name,a.shape,"finite",np.isfinite(a).all(),"range",np.nanmin(a) if a.size else 0,np.nanmax(a) if a.size else 0,flush=True)
        break
