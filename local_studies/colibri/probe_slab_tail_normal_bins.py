...
for name, reduction, cap in (
    ("reduced_predictive", True, 0.005),
    ("raw_predictive", False, 0.005),
    ("raw_default", False, None),
):
    ...
    for step in (744, 745, 746):
        ...
        for repeat in range(2):
            ...
            if name == "reduced_predictive" and step == 746 and repeat == 0:
                ...
                np.savez(
                    "/tmp/colibri_slab_tail_normal_bins_746_reducer.npz",
            ...
            if repeat == 0:
                np.savez(
                    f"/tmp/colibri_slab_tail_normal_bins_pair_{step}_{name}.npz",
                    point0=p0,
                    point1=p1,
                    normals=normals,
                    shapes=shapes,
...
target = [r for r in records if r["step"] == 746 and r["variant"] == "reduced_predictive"]
assert all(r["transported_post_min_gap_m"] < -0.002 for r in target), target
assert all(r["same_input_witness_set_identical"] for r in records), records
print(json.dumps(records, indent=2))
with open("/tmp/colibri_slab_tail_normal_bins_frozen_coverage.json", "w") as stream:
