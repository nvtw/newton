...
a=p.parse_args()
if a.temporal: install_temporal_schedule()
if a.general_direct: solver_module.find_full_coordinate_revolute_trees=lambda *args: []
...
for n in (range(a.start,a.body_count+1) if a.stages else [a.body_count]):
    ...
    print("FAIL_DEPTH" if worst_depth>.001 else "PASS",vars(a),"seconds",elapsed,"fps_with_checks",(frame+1)/elapsed,flush=True)
    del e
    gc.collect()
