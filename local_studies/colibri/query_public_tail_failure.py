...
def audit(example,args):
    ...
    for q,t in zip(poses,times):
        ...
        wp.launch(_contact_separation,n,inputs=[example.state_0.body_q,example.model.shape_body,contacts.rigid_contact_shape0,contacts.rigid_contact_shape1,contacts.rigid_contact_point0,contacts.rigid_contact_point1,contacts.rigid_contact_normal,contacts.rigid_contact_margin0,contacts.rigid_contact_margin1],outputs=[out],device=example.model.device)
        gaps=out.numpy()[:n]; a=contacts.rigid_contact_shape0.numpy()[:n]; b=contacts.rigid_contact_shape1.numpy()[:n]
        p0=contacts.rigid_contact_point0.numpy()[:n];p1=contacts.rigid_contact_point1.numpy()[:n];norm=contacts.rigid_contact_normal.numpy()[:n]
        ...
        for k in order:
            ...
            if len(rows)<15 or any('Tail' in x or 'Cylinder_33' in x for x in labels):
                rows.append(dict(shape_ids=[int(a[k]),int(b[k])],labels=labels,gap=float(gaps[k]),point0=p0[k].tolist(),point1=p1[k].tolist(),normal=norm[k].tolist()))
