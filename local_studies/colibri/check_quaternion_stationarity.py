...
def repeat(qin: wp.array[wp.quatf], out: wp.array2d[wp.quatf], omega: wp.float32):
    ...
    out[i, 0] = q
    for k in range(512):
        q = integrate_orientation(q, wp.vec3f(0.0, 0.0, omega), wp.float32(1.0 / 3600.0))
        if k < 4:
            out[i, k + 1] = q
    out[i, 5] = q
...
def main():
    ...
    q = np.vstack([birth["orientation"][1], native["initial_q"][0, 3:], native["q_history"][:, 0, 3:]]).astype(
        np.float32
    )
    report = {}
    for omega in (0.0, 1e-8, 1e-6):
        out = wp.zeros((len(q), 6), dtype=wp.quatf, device="cpu")
        wp.launch(
            repeat, dim=len(q), inputs=[wp.array(q, dtype=wp.quatf, device="cpu"), out, wp.float32(omega)], device="cpu"
        )
        values = out.numpy()
        ...
        report[str(omega)] = {
            "count": len(q),
            "changed_each_interval": np.sum(changes, axis=0).tolist(),
            "birth_values": values[0].tolist(),
            "max_component_change": float(np.max(abs(values[:, -1] - q))),
            "norm_min": float(norm.min()),
            "norm_max": float(norm.max()),
            "cycles_2": int(
                np.sum(
                    np.all(values[:, 1].view(np.uint32) == values[:, 3].view(np.uint32), axis=1)
    ...
    report["native_history"] = {
        "norm_min": float(np.linalg.norm(q[2:].astype(float), axis=1).min()),
        "norm_max": float(np.linalg.norm(q[2:].astype(float), axis=1).max()),
    }
    report["scope"] = (
        "Actual Warp CPU FP32 polynomial helper; zero omega matches native final normalization. GPU rounding not established. Nonzero helper is copy-prediction polynomial, not full momentum-preserving native integration."
    )
