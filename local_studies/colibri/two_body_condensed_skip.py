"""Exact zero-impulse inactive-contact shortcut; independent of sum-tree changes."""
    ...
    for _sweep in range(sweeps):
        for point in range(count):
            ...
            if lane < 12:
                cn = c[point, 0, lane]
            vn = bias[row]
            # Preserve the original increasing-DOF dot accumulation, avoiding
            # parallel tree-reduction arithmetic changes.
            for dof in range(12):
                coeff = shuffle(cn, dof)
                vi = shuffle(v, dof)
                if lane == 0:
                    vn += coeff * vi
            ...
            vt2 = bias[row + 2]
            for dof in range(12):
                coeff1 = shuffle(ct1, dof)
                coeff2 = shuffle(ct2, dof)
                vi = shuffle(v, dof)
                if lane == 0:
                    vt1 += coeff1 * vi
                    vt2 += coeff2 * vi
