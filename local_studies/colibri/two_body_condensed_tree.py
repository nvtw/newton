"""Fixed-tree FP64 warp reductions; physical accuracy gates replace byteidentity."""
    ...
    for _sweep in range(sweeps):
        for point in range(count):
            ...
            if lane < 12:
                ...
                gt1 = g[point, 1, lane]
                gt2 = g[point, 2, lane]
            # A fixed FP64 tree changes only dot-sum rounding, never precision.
            vn = bias[row] + reduce16(cn * v)
            dn = wp.float64(0)
            if lane == 0:
                oldn = impulse[row]
            ...
            dn = shuffle(dn, 0)
            v += gn * dn
            vt1 = bias[row + 1] + reduce16(ct1 * v)
            vt2 = bias[row + 2] + reduce16(ct2 * v)
            delta1 = wp.float64(0)
            delta2 = wp.float64(0)
            if lane == 0:
                old1 = impulse[row + 1]
