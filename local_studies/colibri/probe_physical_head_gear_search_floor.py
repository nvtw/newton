...
def main():
    ...
    parser.add_argument("--output", default="/tmp/physical_head_gear_search_floor.json")
    args = parser.parse_args()
    trace = np.load("/tmp/physical_head_gear_generation_pose.npz")
    pre, post, qd = trace["pre_q"], trace["post_q"], trace["pre_qd"]
    builder = build_scene(
        body_count=36,
        fix_base=False,
        contact_gap=0.001,
    ...
    state = model.state()
    baseline = np.load("/tmp/physical_head_gear_generation_raw.npz")
    records = []
    capacity = 262144
    for floor in (0.0, 0.002):
        for reduced in (False, True):
            ...
            selected = np.flatnonzero(np.all(shapes == [48, 50], axis=1))
            ...
            record = {
                ...
                "pair_search_gap": search[[48, 50]].tolist(),
