...
q = wp.transform(wp.vec3(0, 0, 1), wp.quat_identity())
body = b.add_link(xform=q, is_kinematic=True)
b.add_shape_sphere(body, radius=0.01)
j = b.add_joint_free(body)
...
for i in range(2):
    p.collide(st, c)
    s.step(st, st, m.control(), c, 1 / 60)
    print(i, st.body_q.numpy(), st.body_qd.numpy(), flush=True)
