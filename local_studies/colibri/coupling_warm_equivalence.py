...
def instrument(*args, **kwargs):
    ...
    for a, s in zip(arrays, saved):
        wp.copy(a, s)
    wp.launch(
        raw_warm, dim=1, inputs=[w._direct_contact_response.data, w._contact_container, w.bodies], device=w.device
    )
    d.solve(use_bias=False)
    ...
    print(
        ...
        np.max(abs(error)),
        "reference",
        np.max(abs(reference)),
        "projected",
        np.max(abs(projected)),
        flush=True,
    )
    ...
    np.savez(
        "/tmp/colibri_cube_warm_equivalence.npz", before=before, reference=reference, projected=projected, error=error
    )
    raise SystemExit
