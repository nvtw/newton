...
class TestInterleavedContactConservation(unittest.TestCase):
    def test_interleaved_copies_conserve_through_moving_com_relaxation(self):
        """Check interleaved contact copies with all original physical assertions."""
            ...
            for width in (1, 4):
                with self.subTest(reverse=reverse, width=width):
                    ...
                    if width == 1:
                        ...
                        # Interleaving places both four-point manifolds in both groups.
                        np.testing.assert_array_equal(dynamic_counts, np.full(3, 2))
