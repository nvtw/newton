...
shape_q = e.model.shape_transform.numpy()


def point(q, p):
    return np.array(wp.transform_point(wp.transform(wp.vec3(q[:3]), wp.quat(q[3:])), wp.vec3(p)))


def inverse_point(q, p):
    return np.array(wp.transform_point(wp.transform_inverse(wp.transform(wp.vec3(q[:3]), wp.quat(q[3:]))), wp.vec3(p)))


meshes = {}
for k, src in enumerate(builder.shape_source):
    if hasattr(src, "vertices"):
        meshes[k] = trimesh.Trimesh(
            vertices=np.array(src.vertices), faces=np.array(src.indices).reshape(-1, 3), process=False
        )
for frame in range(4):
    e.collision_pipeline.collide(e.state_0, e.contacts)
    count = int(e.contacts.rigid_contact_count.numpy()[0])
    s0 = e.contacts.rigid_contact_shape0.numpy()[:count]
    s1 = e.contacts.rigid_contact_shape1.numpy()[:count]
    p0 = e.contacts.rigid_contact_point0.numpy()[:count]
    p1 = e.contacts.rigid_contact_point1.numpy()[:count]
    n = e.contacts.rigid_contact_normal.numpy()[:count]
    ...
    print(
        ...
        count,
        "speed",
        np.max(abs(e.state_0.body_qd.numpy())),
        "hypoframe",
        len(report),
        flush=True,
    )
    for gap, k, a, b in report[:3]:
        print(
            ...
            s0[k],
            s1[k],
            e.model.shape_label[s0[k]],
            e.model.shape_label[s1[k]],
            "world",
            a,
            b,
            flush=True,
        ...
        for sid, pt in ((s0[k], b), (s1[k], a)):
            if sid not in meshes:
                ...
                print(
                    "CYL", sid, "scale", scale, "point", local, "radial", radial, "axial", axial, "sdf", sdf, flush=True
                )
                continue
            bidx = shape_body[sid]
            local = inverse_point(shape_q[sid], inverse_point(q[bidx], pt))
