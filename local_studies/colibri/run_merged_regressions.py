    ...
    try:
        for index, module in enumerate(args.modules):
            ...
            command = [
                ...
                sys.executable,
                "-m",
                "local_studies.colibri.run_merged_friction",
                "unittest",
                "-v",
                module,
            ]
            ...
            manifest = log.with_suffix(".source.json")
            environment = dict(os.environ, COLIBRI_MERGED_MANIFEST=str(manifest))
            with log.open("w") as stream:
                process = subprocess.run(
                    command, cwd=root, stdout=stream, stderr=subprocess.STDOUT, check=False, env=environment
                )
            ...
            skipped_match = re.search(r"skipped=(\d+)", "\n".join(summaries))
            skipped = int(skipped_match.group(1)) if skipped_match else 0
            unchanged = fingerprints() == original
            metadata = json.loads(manifest.read_text()) if manifest.exists() else {}
            staged_ok = metadata.get("stage_unchanged", False) and metadata.get("production_unchanged", False)
            case = {
                "module": module,
                "command": command,
                "log": str(log),
                ...
                "passed": process.returncode == 0 and test_count > skipped and staged_ok,
                "skipped": skipped,
                "staged_manifest": str(manifest),
            ...
            if not case["passed"] and not args.keep_going:
                break
            if time.perf_counter() - batch_start >= args.max_seconds or (args.stop_file and args.stop_file.exists()):
                report["paused_at_module_boundary"] = True
                break
