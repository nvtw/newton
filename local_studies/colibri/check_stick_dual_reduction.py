...
from local_studies.colibri.physical_face_support import factor,modes,audit
...
def main():
    ...
    def response(x):
        ...
        for p in range(len(lam)):
            if a["gamma"][p]>0:
                lam[p,0]=max(0.,-gradient[p,0]/a["gamma"][p])
            else:
    ...
    result=least_squares(lambda x:response(x)[0]*1e6,initial,jac="3-point",
                         xtol=1e-14,ftol=1e-14,gtol=1e-14,max_nfev=100)
    ...
    cone=minimize(lambda x:.5*np.dot(x,x),trial0,jac=lambda x:x,method="SLSQP",
                  constraints=constraints,options=dict(maxiter=200,ftol=1e-14))
    lam[ids,1:]=cone.x.reshape(-1,2)*scale
    report=dict(stick=ids.tolist(),singular=s.tolist(),compatibility=float(np.max(abs(e@yp+q))),
                null_response=float(np.max(abs(e@null))),physical_residual=float(np.max(abs(residual))),
                evaluations=int(result.nfev),cone_status=str(cone.message),
                full_response=float(np.max(abs(y-f.T@lam.ravel()))),audit=audit(a,lam.ravel()))
    certificate=[]
