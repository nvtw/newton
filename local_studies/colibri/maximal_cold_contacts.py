...
for n in (range(a.start,a.body_count+1) if a.stages else [a.body_count]):
    ...
    dispatcher_type=type(e.solver.world._dispatcher)
    original_solve=dispatcher_type.solve
    def cold_solve(self,idt):
        self._world._contact_container.impulses.zero_()
        original_solve(self,idt)
    dispatcher_type.solve=cold_solve
