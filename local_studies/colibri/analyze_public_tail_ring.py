...
for slot in np.argsort(d['step_ids']):
    ...
    for k in range(count):
        ...
        rows.append(dict(index=k, shapes=names, pre_gap=gap[0], post_gap=gap[1], impulse=d['impulses'][slot, k].tolist()))
    pinion = labels.index('TailPinion')
    joint = next(r for r in measure(post, labels)['joints'] if r['joint'] == 'TailMount/TailPinion')
    records.append(dict(step=step, time=(step+1)/120, count=count, joint=joint,
...
for r in records[-20:]:
    ...
    print(r['step'], r['joint']['axis_error_rad'], r['pinion_omega'],
          worst['shapes'], worst['pre_gap'], worst['post_gap'],
          strongest['shapes'], strongest['impulse'][0])
