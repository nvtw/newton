...
try:
    ...
    gate = {k: data[k].dtype == ref[k].dtype and data[k].shape == ref[k].shape and data[k].tobytes() == ref[k].tobytes() for k in ref.files}
    assert all(gate.values()), gate
    assert len(slots) == 2, len(slots)
    pre = slots[-1][0].numpy()
    preqd = slots[-1][1].numpy()
    post = data["q"]
    np.savez("/tmp/physical_head_gear_generation_pose.npz", pre_q=pre, pre_qd=preqd,
             first_half_q=slots[0][0].numpy(), post_q=post)
    model = owner[0].model
    state = model.state()
    ...
    for reduction in (False, True):
        pipeline = newton.CollisionPipeline(
            ...
            reduce_contacts=reduction, rigid_contact_max=65536, contact_matching="disabled",
            speculative_contact_velocity_filter=False, speculative_contact_gap_max=0.005,
        )
        ...
        for pose_name, q, qd in (("generation", pre, preqd), ("failed", post, data["qd"])):
            ...
            assert n < 65536
            shapes = np.column_stack([contacts.rigid_contact_shape0.numpy()[:n], contacts.rigid_contact_shape1.numpy()[:n]])
            ...
            margins = np.column_stack([contacts.rigid_contact_margin0.numpy()[:n], contacts.rigid_contact_margin1.numpy()[:n]])
            ...
            np.savez("/tmp/physical_head_gear_" + label + ".npz", shapes=shapes,
                     point0=p0, point1=p1, normals=normals, margins=margins,
                     query_gap=local_gap, transported_post_gap=post_gap)
            records.append(dict(variant=label, count=n, query_min_gap=float(local_gap.min()) if n else None,
                                post_min_gap=float(post_gap.min()) if n else None,
                                post_deeper_than_1mm=int(np.count_nonzero(post_gap < -.001))))
    assert physical_gap.tobytes() == model.shape_gap.numpy().tobytes()
    report=dict(gate=gate, generation_time_s=0.14166666666666666, failed_time_s=.15,
                records=records, physical_gaps_unchanged=True,
