...
for n in range(a.start, a.body_count + 1) if a.stages else [a.body_count]:
    ...
    original_solve = direct.solve
    saved_bias = wp.empty_like(direct.row_bias)

    def exact_relax(*, use_bias):
        if use_bias:
            original_solve(use_bias=True)
        else:
            wp.copy(saved_bias, direct.row_bias)
            direct.row_bias.zero_()
            original_solve(use_bias=True)
            wp.copy(direct.row_bias, saved_bias)

    direct.solve = exact_relax
