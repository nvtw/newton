...
def main():
    ...
    def ph(p):
        return {k.split(".", 1)[1]: a[k] for k in a.files if k.startswith(p + ".")}
    ...
    l = solved["lambdas"]
    W = np.zeros((6, 6))
    W[:3, :3] = np.eye(3) * solved["inverse_mass"][1]
    ...
    for col in range(int(solved["column_count"][0])):
        ...
        for k in range(h[5, col], h[5, col] + h[6, col]):
            n = l[:3, k].astype(float)
            t = l[3:6, k].astype(float)
    ...
    def base_twist(state):
        start = int(state["copy_section_end"][0])
        end = int(state["copy_section_end"][1])
        if end > start:
            return np.r_[
                state["copy_velocity"][start:end].astype(float).mean(0),
                state["copy_angular_velocity"][start:end].astype(float).mean(0),
            ]
    ...
    for j in range(int(a["num_joints"][0])):
        for side, key in [(0, "joint_wrench0"), (1, "joint_wrench1")]:
            ...
            for i in range(solved["joint_row_count"][j]):
                ...
                joint += solved[key][st, local].astype(float) * (
                    float(solved["joint_accumulated"][row]) - float(before["joint_accumulated"][row])
                )
    ...
    A = J @ W @ J.T
    rhs = J @ free
    mc = 0.9417003989219666
    ic = 0.05829954519867897
    reports = {}
    for name, regularization in [("rigid_unbiased", np.zeros(6)), ("native_soft_unbiased", ic / mc * np.diag(A)[::3])]:
        ...
        if name == "rigid_unbiased":
            ...
            tries.append(
                dict(
                    method="inscribed_cone_sticking_feasibility_64_rays",
                    success=lp.success,
        ...
        ke = 0.5 * (v @ M @ v - free @ M @ free)
        work = 0.5 * (v + free) @ impulse
        omitted = {str(k): (points[k][0] @ v).tolist() for k in points if k not in selected and points[k][3] <= 0}
        reports[name] = dict(
            residual=residual,
            tries=tries,
            impulses=triples.tolist(),
            velocity=v.tolist(),
            max_tangent_speed=float(np.linalg.norm(vel[:, 1:], axis=1).max()),
            normal_total_Ns=float(triples[:, 0].sum()),
            friction_capacity_sum_Ns=float(mu @ triples[:, 0]),
            cone_violation=float(np.max(np.linalg.norm(triples[:, 1:], axis=1) - mu * triples[:, 0])),
            energy_change_J=float(ke),
            midpoint_work_J=float(work),
            work_error_J=float(abs(ke - work)),
            P_error=float(np.max(abs(impulse[:3] + ground_linear))),
            L_error=float(np.max(abs(body_angular + ground_angular))),
            omitted_non_spec_velocities=omitted,
            min_omitted_normal=float(min(x[0] for x in omitted.values())),
        )
        np.savez(
            "/tmp/colibri_frozen_base_" + name + ".npz",
            J=J,
            W=W,
            rhs=rhs,
    ...
    result = dict(
        source="/tmp/colibri_support_phases330.npz",
        ...
        external_joint_impulse=joint.tolist(),
        native_after=vafter.tolist(),
        free_twist=free.tolist(),
        phase_reconstruction_max_error=error,
        results=reports,
