...
def main():
    ...
    source = Path("/tmp/colibri_total_normal_support_phases330.npz")
    arrays = np.load(source)
    d = {k.split(".", 1)[1]: arrays[k] for k in arrays.files if k.startswith("biased_solved.")}
    ...
    for j in joints:
        ...
        for row in d["joint_row_indices"][j, : int(d["joint_row_count"][j])]:
            ...
            targets.append(float(d["joint_reference"][row]) if dynamic else -float(d["joint_bias"][st, local]))
    ...
    for col in range(int(d["column_count"][0])):
        ...
        for p in range(h[5, col], h[5, col] + h[6, col]):
            ...
            regularization.append(0.0 if d["derived"][3, p] > 0 else ic / (mc * float(d["derived"][0, p])))
    ...
    Path("/tmp/colibri_total_normal_support_frozen.json").write_text(json.dumps(report, indent=2))
    np.savez(
        "/tmp/colibri_total_normal_support_frozen.npz",
        W=W,
        B=B,
        C=C,
        A=A,
