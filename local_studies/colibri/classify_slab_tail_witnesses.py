...
result = {}
for name, mask in [
    ("all", np.ones(len(gap), bool)),
    ("future_penetrating", future),
    ("future_predicted_impact", future & (pred < 0)),
    ("future_initially_receding", future & (vn >= 0)),
]:
    ids = np.flatnonzero(mask)
    result[name] = {
        "count": len(ids),
        "pre_gap_range": [float(gap[ids].min()), float(gap[ids].max())] if len(ids) else None,
        "vn_range": [float(vn[ids].min()), float(vn[ids].max())] if len(ids) else None,
        "pred_gap_range": [float(pred[ids].min()), float(pred[ids].max())] if len(ids) else None,
        "max_dot_reduced_range": [float(maxdot[ids].min()), float(maxdot[ids].max())] if len(ids) else None,
    }
result["worst_future_samples"] = [
    {
        "id": int(i),
        "pre_gap": float(gap[i]),
        "vn": float(vn[i]),
        "pred_gap": float(pred[i]),
        "post_gap": float(after[i]),
        "normal": raw["normals"][i].tolist(),
        "max_dot_reduced": float(maxdot[i]),
    }
    for i in np.argsort(after)[:8]
]
