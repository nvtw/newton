...
def initialize(self, *pos, **kw):
    ...
    def record(label, full=True):
        ...
        if label not in slots:
            slots[label] = {name: wp.empty_like(arrays[name]) for name in names}
        for name, target in slots[label].items():
            wp.copy(target, arrays[name])
    ...
    captured.update(slots=slots, world=w, model=self.model)
            ...
            json.dumps(
                {
                    ...
                    "scope": "Last temporal substep phase snapshots; separate wp.copy launches; no arithmetic kernel changes; all-frame byte gate required",
                },
                indent=2,
            )
        )
