...
for n in range(a.start, a.body_count + 1) if a.stages else [a.body_count]:
    ...
    assert e.solver._maximal_contact_response is not None
        ...
        for side in (0, 1):
            ...
            sides.append(
                np.array(
                    [
                        np.array(
                            wp.transform_point(wp.transform(q[shape_body[s], :3], q[shape_body[s], 3:]), wp.vec3(pt))
                        )
                        if shape_body[s] >= 0
                        else pt
                        for s, pt in zip(shapes, points)
                    ]
                )
            )
        gaps = np.einsum("ij,ij->i", sides[1] - sides[0], fresh.rigid_contact_normal.numpy()[:count])
        worst_depth = max(worst_depth, float(-gaps.min()))
        if frame % 10 == 0 or gaps.min() < -0.001:
            k = int(gaps.argmin())
            print(
                "FRESH_DEPTH",
                frame,
                ...
                worst_depth,
                "shapes",
                *[e.model.shape_label[getattr(fresh, f"rigid_contact_shape{side}").numpy()[k]] for side in (0, 1)],
                flush=True,
            )
        if gaps.min() < -0.001:
            break
    ...
    print(
        "FAIL_DEPTH" if worst_depth > 0.001 else "PASS",
        vars(a),
        "seconds",
        elapsed,
        "fps_with_checks",
