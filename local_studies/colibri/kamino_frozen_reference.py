...
for (r, c), value in zip(d['jacobian_nzb_coords'][:int(d['jacobian_num_nzb'][0])], d['jacobian_nzb_values']):
    J[r, c:c+6] += value
M = block_diag(*[block_diag(float(m)*np.eye(3), I.astype(float)) for m, I in zip(d['bodies_inv_m'], d['bodies_inv_I'])])
A = J @ M @ J.T
...
print('full_velocity_error', np.max(abs(velocity-d['solution_v_plus'][:n])), 'eq_residual', np.max(abs(velocity[:b])), flush=True)
print('rhs_error', np.max(abs(J @ d['problem_u_f'].ravel()+d['problem_v_b'][:n]+d['problem_v_i'][:n]-f)), flush=True)
np.savez('/tmp/colibri_zero_bias_schur_reference.npz', J=J, inverse_mass=M, A=A, B=B, S=S, q=q, f=f, lambdas=lam, velocity=velocity, regularization=regularization, mu=d['problem_mu'][:k//3], cached_response=R, cached_coupling=C)
