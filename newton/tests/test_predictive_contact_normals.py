...
@wp.kernel
def _compare_predictive_normal_paths(
...
def test_predictive_normal_allocation_invariance(test, device, deterministic):
    count = 128
    ...
    reused = GlobalContactReducer(capacity=2 * count, device=device, deterministic=deterministic)
    allocated = GlobalContactReducer(capacity=2 * count, device=device, deterministic=deterministic)
    indices = wp.full((count, 2), -1, dtype=wp.int32, device=device)
    wp.launch(
        _compare_predictive_normal_paths,
        count,
        [
    ...
    ids = indices.numpy()
    test.assertTrue(np.all(ids > 0))
    a = reused.normal.numpy()[ids[:, 0]]
    b = allocated.normal.numpy()[ids[:, 1]]
    # Reusing a regular-reduction contact must preserve the same encoded normal
    # as allocating storage only after predictive reduction accepts it.
    np.testing.assert_array_equal(a, b)
    np.testing.assert_array_equal(reused.position_depth.numpy()[ids[:, 0]], allocated.position_depth.numpy()[ids[:, 1]])
    np.testing.assert_array_equal(reused.shape_pairs.numpy()[ids[:, 0]], allocated.shape_pairs.numpy()[ids[:, 1]])


class TestPredictiveContactNormals(unittest.TestCase):
    pass


for deterministic in (False, True):
    add_function_test(
        TestPredictiveContactNormals,
        f"test_predictive_normal_allocation_invariance_{deterministic}",
        test_predictive_normal_allocation_invariance,
