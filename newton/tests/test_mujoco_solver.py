...
class TestMuJoCoConversion(unittest.TestCase):
    ...
    def test_separate_envs_to_worlds_false_multi_env_validation(self):
        """Test that separate_envs_to_worlds=False is rejected for multi-environment models."""
        # Create a model with 2 environments
        template_builder = newton.ModelBuilder()
        ...
        for i in range(2):
            ...
            builder.add_builder(template_builder, xform=env_transform, update_num_env_count=True)

        model = builder.finalize()
        self.assertEqual(model.num_envs, 2, "Model should have 2 environments")

        # Test that separate_envs_to_worlds=False raises ValueError
        with self.assertRaises(ValueError) as context:
            SolverMuJoCo(model, separate_envs_to_worlds=False)

        self.assertIn("separate_envs_to_worlds=False", str(context.exception))
        self.assertIn("single-environment", str(context.exception))
        self.assertIn("num_envs=2", str(context.exception))

        # Test that separate_envs_to_worlds=True works fine
        solver = SolverMuJoCo(model, separate_envs_to_worlds=True)
        self.assertIsNotNone(solver)

    def test_separate_envs_to_worlds_false_single_env_works(self):
        """Test that separate_envs_to_worlds=False works correctly for single-environment models."""
        builder = newton.ModelBuilder()
        ...
        # Should work fine with single environment
        solver = SolverMuJoCo(model, separate_envs_to_worlds=False)
        self.assertIsNotNone(solver)
        self.assertEqual(solver.mj_model.nv, 1)
