...
# ---------------------------------------------------------------------------
# Off-origin convex hull regression
# ---------------------------------------------------------------------------
# The MPR/GJK initial interior point ``v0`` of the Minkowski difference is
# computed by ``geometric_center`` in ``newton/_src/geometry/mpr.py``.  For
# ``CONVEX_MESH`` shapes whose authoring origin lies outside the hull
...
# where all triangle supports collapse onto a single vertex of the partner.
# MPR then converges on a wrong normal/penetration even when the two shapes
# are clearly separated.
#
# This test pins one such configuration: a small triangle from a non-convex
# mesh against a thin convex hull whose local-frame origin sits ~17 cm
# outside its AABB.  In world space the two shapes are separated by
# ~1.42 mm along Z.  Without the AABB-center initialization in
# ``geometric_center``, MPR reports ~7.1 mm penetration along +Y; with it,
# MPR reports the correct +1.42 mm separation along -Z.

# Triangle from a non-convex mesh; vertices are very small (sub-cm).
_OFF_ORIGIN_TRIANGLE_VERTICES_LOCAL = np.array(
    [
...
_OFF_ORIGIN_CONVEX_HULL_VERTICES_LOCAL = np.array(
    [
...
_OFF_ORIGIN_CONVEX_HULL_INDICES = np.array(
    [
        [7, 1, 0],
        [7, 0, 6],
        [7, 4, 5],
...
_OFF_ORIGIN_TRIANGLE_BODY_XFORM: tuple[tuple[float, float, float], tuple[float, float, float, float]] = (
    (0.18269123136997223, -0.1676918864250183, 0.18899677693843842),
    (0.0, 0.0, 0.0, 1.0),
)
_OFF_ORIGIN_HULL_BODY_XFORM: tuple[tuple[float, float, float], tuple[float, float, float, float]] = (
    (0.0, 0.0, 0.20000000298023224),
    (0.0, 0.0, 0.0, 1.0),
)

_OFF_ORIGIN_PER_SHAPE_GAP = 0.001  # m
_OFF_ORIGIN_EXPECTED_SIGNED_GAP = 0.0014239252  # m, derived from AABB Z-separation
_OFF_ORIGIN_EXPECTED_NORMAL_Z = -1.0  # contact normal should point along -Z
...
_OFF_ORIGIN_GAP_TOL = 5.0e-5
_OFF_ORIGIN_NORMAL_TOL = 1.0e-3


def _build_off_origin_pair_model(
    device,
) -> tuple[newton.Model, newton.State, dict[str, int]]:
    """Build a two-shape model with a single triangle vs an off-origin hull."""
    triangle_mesh = newton.Mesh(
        _OFF_ORIGIN_TRIANGLE_VERTICES_LOCAL.copy(),
        np.array([0, 1, 2], dtype=np.int32),
        compute_inertia=False,
        is_solid=False,
    )
    hull_mesh = newton.Mesh(
        _OFF_ORIGIN_CONVEX_HULL_VERTICES_LOCAL.copy(),
        _OFF_ORIGIN_CONVEX_HULL_INDICES.copy(),
        compute_inertia=False,
        is_solid=False,
    )

    builder = newton.ModelBuilder()
    builder.default_shape_cfg.gap = _OFF_ORIGIN_PER_SHAPE_GAP
    builder.default_shape_cfg.is_hydroelastic = False

    triangle_body = builder.add_body(
        xform=_OFF_ORIGIN_TRIANGLE_BODY_XFORM,
        is_kinematic=True,
        label="triangle_body",
    )
    hull_body = builder.add_body(
        xform=_OFF_ORIGIN_HULL_BODY_XFORM,
        is_kinematic=True,
        label="hull_body",
    )
...
def _collect_off_origin_signed_gaps(
...
def test_off_origin_convex_hull_reports_correct_separation(test, device):
    ...
    with test.subTest(device=str(device)):
        model, state, shapes = _build_off_origin_pair_model(device)
        ...
        gaps, normals = _collect_off_origin_signed_gaps(contacts, model, state, shapes["triangle"], shapes["hull"])
        ...
        test.assertAlmostEqual(
            signed_gap,
            _OFF_ORIGIN_EXPECTED_SIGNED_GAP,
            delta=_OFF_ORIGIN_GAP_TOL,
            msg=(
        ...
        test.assertAlmostEqual(
            float(mean_normal[2]),
            _OFF_ORIGIN_EXPECTED_NORMAL_Z,
            delta=_OFF_ORIGIN_NORMAL_TOL,
