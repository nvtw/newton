...
class TestSelection(unittest.TestCase):
    ...
    def _test_selection_shapes(self, floating: bool):
        ...
        self.assertEqual(single_ant_view.get_attribute("shape_gap", single_ant_model).shape, (1, 1, S))
        ...
        self.assertEqual(
            single_ant_per_world_view.get_attribute("shape_gap", single_ant_per_world_model).shape, (W, 1, S)
        )
        ...
        self.assertEqual(
            multi_ant_per_world_view.get_attribute("shape_gap", multi_ant_per_world_model).shape, (W, A, S)
        )
    ...
    def test_selection_shape_values_noncontiguous(self):
        ...
        # Each link has one shape with a distinct gap value
        robot = newton.ModelBuilder()

        gaps = [0.001, 0.002, 0.003]
        ...
        robot.add_shape_box(
            ...
            hy=0.1,
            hz=0.1,
            cfg=newton.ModelBuilder.ShapeConfig(gap=gaps[0]),
            label="shape_base",
        )
        ...
        robot.add_shape_capsule(
            ...
            radius=0.05,
            half_height=0.2,
            cfg=newton.ModelBuilder.ShapeConfig(gap=gaps[1]),
            label="shape_link1",
        )
        ...
        robot.add_shape_sphere(
            link2,
            radius=0.05,
            cfg=newton.ModelBuilder.ShapeConfig(gap=gaps[2]),
            label="shape_link2",
        )
        ...
        # read shape_gap through ArticulationView and check values
        vals = view.get_attribute("shape_gap", model)
        self.assertEqual(vals.shape, (W, 1, 2))
        vals_np = vals.numpy()

        expected = [gaps[0], gaps[2]]  # base and link2 (link1 excluded)
        for w in range(W):
            for s, expected_gap in enumerate(expected):
