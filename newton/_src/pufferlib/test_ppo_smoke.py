...
class TestPPOSmoke(unittest.TestCase):
    ...
    def test_cartpole_discrete(self):
        """CartPole (discrete, 4D obs, 2 actions) trains without error."""
    ...
    def test_drone_continuous(self):
        """Drone (continuous, 23D obs, 4 actions) trains without error."""
        from newton._src.pufferlib.envs.drone import DroneEnv

        N = 256
        self._train_env(
            lambda d: DroneEnv(num_envs=N, device=d, seed=42),
            dict(
                num_envs=N, horizon=32, obs_dim=23, num_actions=4, hidden=32,
                lr=0.005, continuous=True, init_logstd=0.0, activation="elu",
                replay_ratio=1.0, minibatch_size=N * 32,
            ),
        )

    def test_squared_continuous(self):
        """SquaredContinuous (1D obs, 1 action) trains without error."""
        from newton._src.pufferlib.envs.squared_continuous import SquaredContinuousEnv

        N = 256
        self._train_env(
            lambda d: SquaredContinuousEnv(num_envs=N, device=d, seed=42),
            dict(
                num_envs=N, horizon=32, obs_dim=1, num_actions=1, hidden=32,
                lr=0.005, continuous=True, init_logstd=0.0,
                replay_ratio=1.0, minibatch_size=N * 32,
            ),
        )

    def test_anymal_env_init_and_step(self):
        """AnymalEnv initializes and steps without error (4 envs, mujoco contacts)."""
    ...
    def test_anymal_training(self):
        """AnymalEnv trains for a few iterations without error."""
        from newton._src.pufferlib.envs.anymal import AnymalEnv
