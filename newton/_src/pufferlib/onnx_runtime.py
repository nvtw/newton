...
Example::
    ...
    from newton._src.pufferlib.onnx_runtime import OnnxRuntime
...
from newton._src.pufferlib.kernels import matmul as tiled_gemm  # noqa: F401 — re-export
...
# ---------------------------------------------------------------------------
# Standalone tiled GEMM dispatch (usable outside OnnxRuntime)
# ---------------------------------------------------------------------------
...
def tiled_gemm(
    ...
    """Launch a tiled GEMM: ``C = op(A) @ op(B)``.

    Automatically picks tile sizes that work for any matrix dimension.
    Supports ``transpose_a`` and ``transpose_b`` independently.

    Args:
        A: Input matrix.  Shape ``(M, K)`` or ``(K, M)`` if transposed.
        B: Input matrix.  Shape ``(K, N)`` or ``(N, K)`` if transposed.
        C: Output matrix.  Shape ``(M, N)``.
        transpose_a: If True, treat A as transposed.
        transpose_b: If True, treat B as transposed.
    """
    ...
    if transpose_a and not transpose_b:
        kernel = _make_tiled_gemm_transA_kernel(tile_m, tile_n, tile_k)
        ...
        raise NotImplementedError("tiled_gemm with both transpose_a and transpose_b")
    else:
...
def _launch_tiled_gemm(
    ...
    tiled_gemm(A, B, C, transpose_b=transB)
