...
Kamino now supports mraksha bounded recovery bias as an opt-in cap on penetration correction. Positive-gap contact rows still keep the full opening bias so stale warm-started normal impulses can release instead of pulling bodies together during separation. In the Dr Legs examples `gamma=0.015` keeps standing contact penetration in the same low-millimeter band while avoiding the visible vertical pumping produced by `gamma=0.05`; both keep `delta=1e-3` and the 1 m/s safety cap.
