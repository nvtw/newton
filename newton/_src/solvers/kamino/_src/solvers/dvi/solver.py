...
class DVISolver:
    ...
    def finalize(
        ...
        bounded_counts = model.info.num_bounded_joint_cts.numpy().astype(int).tolist()
        ...
        self._unilateral_strides_host = [
            ...
            for bounded_count, limit_count, contact_count in zip(
                bounded_counts, max_limits, max_contacts, strict=True
            )
        ]
        ...
        if limits is not None and limits.model_max_limits_host > 0:
            wp.launch(
                ...
                inputs=[
                    ...
                    limits.lid,
                    limits.bids,
                    self._sparse_path.model.bodies.effective_inv_m_i,
                    problem.data.lio,
                    problem.data.iio,
                    problem.data.nbc,
                    state.limit_indices,
