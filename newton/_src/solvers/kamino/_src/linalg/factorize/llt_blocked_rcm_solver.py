...
class LLTBlockedRCMSolver(DirectSolver[wp.float32, wp.int32]):
    ...
    def __init__(
        ...
        **kwargs: dict[str, Any],
        ...
        Args:
            ...
            parallel_factorization: Whether to solve each Cholesky panel with independent CUDA blocks.
        """
        ...
        self._parallel_factorization = bool(parallel_factorization)
    ...
    def _reset_impl(self) -> None:
        ...
        self._b_hat.zero_()
        self._x_hat.zero_()
        if not self._fixed_permutation:
            self._P.zero_()
        self._inv_P.zero_()
        self._tile_pattern.zero_()
        self._has_factors = False

    def set_permutation(self, permutation: wp.array[wp.int32]) -> None:
        """Install a topology-derived permutation reused by every factorization."""
        if permutation.dtype != wp.int32 or permutation.ndim != 1:
            raise TypeError("permutation must be a one-dimensional wp.int32 array")
        if permutation.size != self.P.size:
            raise ValueError(f"permutation must contain {self.P.size} entries, got {permutation.size}")
        wp.copy(self.P, permutation)
        self._fixed_permutation = True
        self._reorder_callback = None
    ...
    def _factorize_impl(self, A: wp.array[Any]) -> None:
        ...
        if not self._fixed_permutation:
            # Numerical fallback for callers without immutable topology.
