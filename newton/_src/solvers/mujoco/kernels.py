...
def convert_newton_contacts_to_mjwarp_kernel(
    ...
    rigid_contact_gap0: wp.array(dtype=wp.float32),
    rigid_contact_gap1: wp.array(dtype=wp.float32),
    ...
    # rigid_contact_thickness = radius_eff + shape_gap per shape.
    # Subtract only radius_eff so dist is the surface-to-surface distance.
    # shape_gap is handled by geom_margin (MuJoCo's includemargin threshold).
    radius_eff = (rigid_contact_gap0[tid] - shape_gap[shape_a]) + (
        rigid_contact_gap1[tid] - shape_gap[shape_b]
    )
...
def update_geom_properties_kernel(
    ...
    Note: geom_margin is always updated from shape_gap (unconditionally,
    ...
    # update geom_margin from shape gap
    geom_margin[world, geom_idx] = shape_gap[shape_idx]
