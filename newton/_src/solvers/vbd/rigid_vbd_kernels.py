...
def accumulate_body_body_contacts_per_body(
    ...
    rigid_contact_gap0: wp.array(dtype=float),
    rigid_contact_gap1: wp.array(dtype=float),
    shape_body: wp.array(dtype=wp.int32),
    body_contact_buffer_pre_alloc: int,
    body_contact_counts: wp.array(dtype=wp.int32),
    body_contact_indices: wp.array(dtype=wp.int32),
    ...
    while i < num_contacts:
        ...
        gap_total = rigid_contact_gap0[contact_idx] + rigid_contact_gap1[contact_idx]
        dist = wp.dot(contact_normal, cp1_world - cp0_world)
        penetration = gap_total - dist
...
def update_duals_body_body_contacts(
    ...
    Args:
        ...
        rigid_contact_gap0/1: Per-shape gap (for SDF/capsule padding)
    ...
    gap_total = rigid_contact_gap0[idx] + rigid_contact_gap1[idx]
    penetration = wp.max(0.0, gap_total - dist)
