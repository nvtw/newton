...
def phase_balance_and_step_forward() -> CurriculumPhase:
    ...
    return CurriculumPhase(
        ...
        purpose="Start with an easy forward command so standing, posture, and foot timing stabilize first.",
...
def phase_walk_forward() -> CurriculumPhase:
    ...
    return CurriculumPhase(
        name="walk_forward",
        title="Walk Forward",
        purpose="Increase command speed and reduce the one-direction progress helper.",
        command=FORWARD,
        iterations=220,
        env_overrides=(
            ("action_scale", 0.50),
            ("forward_progress_reward_scale", 0.70),
...
def phase_fast_efficient_forward() -> CurriculumPhase:
    ...
    return CurriculumPhase(
        ...
        purpose="Push the gait toward a useful speed while penalizing excess action and energy.",
        command=FORWARD_FAST,
        iterations=260,
        env_overrides=(
            ("action_scale", 0.50),
            ("forward_progress_reward_scale", 0.30),
...
def phase_robust_forward() -> CurriculumPhase:
    ...
    return CurriculumPhase(
        ...
        purpose="Keep forward walking stable when the base receives realistic noise and occasional kicks.",
        command=FORWARD_FAST,
        iterations=180,
        env_overrides=(
            ("action_scale", 0.50),
...
def phase_reverse_walk() -> CurriculumPhase:
    ...
    return CurriculumPhase(
        ...
        command_zero_probability=0.10,
        eval_commands=(IDLE, FORWARD, BACKWARD),
        gate_min_tracking_perf=0.35,
        gate_max_fall_fraction=0.18,
        gate_min_survival_fraction=0.82,
...
def phase_side_step() -> CurriculumPhase:
    ...
    return CurriculumPhase(
        ...
        eval_commands=(IDLE, FORWARD, LEFT, RIGHT),
...
def phase_full_control_mix() -> CurriculumPhase:
    ...
    return CurriculumPhase(
        ...
        env_overrides=(
            ("action_scale", 0.50),
            ("lin_vel_reward_scale", 2.75),
            ("yaw_rate_reward_scale", 1.40),
            ("forward_progress_reward_scale", 0.05),
            ("energy_reward_scale", -3.0e-5),
            ("action_rate_reward_scale", -0.015),
        ),
        randomize_commands=True,
