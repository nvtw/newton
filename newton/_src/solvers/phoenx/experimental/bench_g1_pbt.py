...
quality gate, and the weakest members can be replaced by mutated copies of the
strongest members.

The objective is deliberately the gate score, not training reward. This avoids
reward-weight inflation and lets shaped rewards act as a search surface rather
...
def _base_trial_config() -> TrialConfigPBT:
    return TrialConfigPBT(
...
def _train_and_score_member(
    ...
    member.score = _gate_score(stats, args)
...
def run_pbt(args: argparse.Namespace) -> dict[str, Any]:
    ...
    for policy_id in range(int(args.population_size)):
        cfg = base if policy_id == 0 else _mutate_config(base, args, rng)
    ...
    for epoch in range(int(args.epochs)):
        ...
        evolve_events = _evolve(population, args, rng)
