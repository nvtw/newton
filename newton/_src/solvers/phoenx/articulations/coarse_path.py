...
"""Factor-2 Galerkin block-GS corrections for path articulations."""
...
def _prolongate_coarse_path_kernel(
    ...
    block_size = block_sizes[fine_global]
    if row < block_size:
        value = wp.float32(0.0)
        if row < rows:
            coarse_base = path * coarse_blocks_per_path
        ...
        active_block = pivot_order[fine_global]
        fine_solution[active_block_offsets[active_block] + row] = value


class CoarsePathSolver:
    """Reusable buffers for packed factor-2 path corrections."""
    ...
    def solve(self, system: ArticulationDeviceSystem, *, device=None) -> None:
        ...
        wp.launch(
            ...
            dim=(self.path_count, self.fine_blocks_per_path * _BLOCK_SIZE),
            inputs=[
                self.solution,
                wp.int32(self.fine_blocks_per_path),
                wp.int32(self.coarse_blocks_per_path),
