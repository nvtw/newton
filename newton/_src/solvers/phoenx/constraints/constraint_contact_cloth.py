...
@wp.func
def _contact_uses_sdf_start_gap(contacts: ContactViews, k: wp.int32) -> bool:
    ...
    if shape0 >= wp.int32(0) and shape0 < shape_type_count:
        ...
        uses_sdf = uses_sdf or type0 == wp.int32(GeoType.MESH) or type0 == wp.int32(GeoType.HFIELD)
    if shape1 >= wp.int32(0) and shape1 < shape_type_count:
        type1 = contacts.shape_type[shape1]
        uses_sdf = uses_sdf or type1 == wp.int32(GeoType.MESH) or type1 == wp.int32(GeoType.HFIELD)
    return uses_sdf
...
def _make_contact_prepare_for_iteration_at(
    ...
    def impl(
        ...
        for i in range(contact_count):
            ...
            # Speculative (>0) vs penetrating (<0) Baumgarte bias. Mesh/SDF
            # rows generated separated keep their generation-time gap as an
            # activation barrier; primitive contacts use the live substep gap so
            # dense stacks can become load-bearing within TGS.
            solver_gap = effective_gap
            use_start_gap = _contact_uses_sdf_start_gap(contacts, k)
            if use_start_gap:
