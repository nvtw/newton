...
| Direct equalities | COM-centered, diagonally equilibrated RCM/LLT with flat valid-entry and panel task lists | A 12-link chain retains about 2.6x legacy PGS FPS while reducing anchor error 13x at equal masses and 286--343x at 10,000:1. A finite heterogeneous 42-mechanism A/B remains throughput-neutral relative to the pre-change direct solver. |
...
| Atomic direct-equality panel stack | CUDA already load-balances independent thread blocks. A precomputed flat list of valid `(mechanism, panel)` tasks avoids rectangular holes without a contended counter or graph-capture-visible mutable state. |
