...
- Fusing the next BF16 activation store into MinGRU removed exactly four casts per replay minibatch (7,680 to 6,144 launches in the bounded trace), but increased producer traffic. A longer reverse bracket measured 1.7497M baseline versus 1.7463M candidate (-0.2%), so the prototype was removed.
- Sharing unchanged actor BF16 weights between mirror and manual forwards also removed four casts per minibatch without extra GPU stores, but a longer matched pair was neutral: 1.7497M baseline versus 1.7493M candidate. The buffer-lifetime machinery was removed rather than retain code for an unmeasurable gain.
- Patch rows often leave most of the 96-thread builder block idle, but compact workers do not improve the critical path: 32 lanes measured 1.7348M samples/s (-0.8%), while 64 lanes measured 1.7515M versus a nearby 1.7493M control (+0.13%). Both looped-worker variants were removed; useful articulation traversal and memory dependencies dominate.
...
- A current-toolchain mini rerun made dense vec4 grouping neutral at 65,536 rows and 1.34x faster at 262,144 rows, while sparse grouping was 2x slower at both sizes. The corresponding full-solver test padded the 21-float symmetric spatial inertia to six vec4-sized groups without changing logical values. Reversed G1 brackets consistently regressed about 0.6–0.7%, because 14% extra footprint outweighed alignment; reverted.

## Deterministic shared rollout diagnostics (2026-07-27)

- `BufferRollout` accumulated reward, done, and success diagnostics with unordered floating-point atomics. These values never feed PPO, explaining why repeated Ant checkpoints were bit-identical while the printed mean reward moved by roughly 1e-5. The same shared reporting path affected every non-G1 caller.
- A fixed-order 32-lane partial reduction plus fixed-order final reduction removes the nondeterminism. A cancellation-sensitive 262,144-sample regression is bitwise stable with the new path; temporarily restoring the old kernel makes it fail, changing an identical reward sum from -102682.390625 to -102684.78125 on the next launch.
- The ordered reduction is also faster at that production-scale count: about 41 us versus 1000 us for the contended atomic kernel over 1,000 warmed reductions. CUDA graph capture and host-copy tests pass.
- A refreshed three-substep full G1 training trace at 8,192 worlds attributes 13.4% of CUDA time to fused advance/publish, 10.1% to packed contact-row construction, 10.0% to the two patch solves, 5.6% to external advance, and 10.1% combined to MinGRU forward/backward. Physics traversal remains the largest optimization surface. The production bytes-per-sector capture is still required before changing hot solver layouts.
- MinGRU sequence backward recomputed the negative candidate sigmoid while evaluating its derivative even though the identical candidate was already live. Reusing it cuts the matched backward median from 114.6 to 109.5 us (-4.5%), about 0.2% projected end-to-end. Randomized 64-step old/new gradient arrays with done resets are bitwise identical, and four graph/finite-difference/BF16 backward tests pass.
- PufferLib delegates every Muon Gram contraction to PyTorch/cuBLAS, while PhoenXRL used Warp scalar/tiled kernels. Routing grouped, wide, and tall Gram products through the existing graph-capturable optional cuBLAS bridge reduces their bounded-trace total from 43.9 to 27.6 ms (-37%). Same-session interleaved update graphs improve 98.18 to 95.30 ms (-2.94%); the Warp fallback is unchanged.
- cuBLAS changes FP32 contraction order but not the algorithm: after ten production-shape Muon steps the maximum parameter delta versus Warp is 3.73e-8. Four wide/tall/grouped/layout parity tests pass, and two same-seed six-update G1 trainings are bitwise identical across all 71 checkpoint arrays. Ant uses Adam by default and is unchanged.
- A longer full-training candidate/control/candidate bracket measures 1.7865M / 1.7769M samples/s around a 1.7780M Warp control. This is neutral within thermal noise: graph leapfrogging hides the faster learner behind rollout. Retain the path for the proven learner gain and future physics headroom, not as a current end-to-end G1 throughput claim.

## Production coalescing counters (2026-07-26)

- The corrected three-substep NCU capture is complete. Advance/publish, packed-row construction, both patch solves, external advance, and factor reach only 20–39% DRAM throughput (factor reaches 61% compute/cache throughput), while dependent L1TEX waits account for 53–75% of issue gaps. Eligible warps remain only 0.17–0.57 per scheduler.
- Nsight reports 4–10 useful bytes per 32-byte sector, but this includes intentional same-address broadcasts and sparse tree lanes; it is not evidence for a blanket vec4 conversion. Always routing packed responses through the existing coalesced transpose confirms the tradeoff: production graph throughput falls from 2.407M to 2.248M samples/s (-6.6%). The three-line prototype was removed.
- The next supported direction is reuse of topology/factor data along the dependent packed-row traversal. Any scratch or staging scheme must amortize its pass in the full production graph; raw transaction efficiency alone is not a success criterion.


## Cooperative patch-row traversal (2026-07-27)

- Production NCU showed the compact patch-row builder dominated by dependent topology/factor loads (238.2 us, 0.20 eligible warps/scheduler), not raw DRAM bandwidth. A direct always-transpose layout lost 6.6%, and applying subgroup-per-row traversal to dense point contacts lost 9.5%; both prototypes were removed.
- The retained patch-only builder projects each row on lane 0, then walks articulation depths cooperatively. Parent spatial responses move through masked subgroup shuffles and remain in registers instead of round-tripping through the per-body global response tensor. The hard-contact and ABA equations are unchanged.
