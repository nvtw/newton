...
| Reduced contacts | Deterministic packed same-articulation point rows | Replaces the world-serial self-contact fallback with a fixed-order two-wrench ABA response. A 4,096-world Humanoid fixed-FlashSAC run improved from about 65.4k to 271.3k transitions/s and reached the sustained walking gate in 83.0 s of training; the previous fixed-policy run needed 375.8 s. |
...
| Joint inequalities | Keep only compact descriptor and diagnostic state in the production joint column | Removing obsolete bilateral Schur caches and projector runtime geometry reduces the column from 116 to 35 dwords, deletes a marker launch and redundant compile-time specializations, and skips duplicate drive and reaction writes. Analytical joint/D6, drive, direct-contact, and projected-tree momentum tests pass. |
| D6 inequality consolidation | Route native BALL and classified D6 inequalities through common six-axis rows, then remove the legacy angular and scalar axial implementations | Deletes duplicate preparation, warm-start, iteration, per-column cache state, and seven obsolete adapter uploads. The common representation removes the legacy D6 mode classifier; together with projector cleanup the production joint column is 35 dwords. Common rows retain paired impulses and support limits, speed caps, friction, and per-axis MuJoCo slip regularization, including live property refresh. Valid noncanonical cylindrical- and planar-shaped axis layouts fall back to generic complement rows instead of being rejected. Three-axis angular rows use reciprocal coordinate gradients at full rank and the Moore-Penrose minimum-norm map at gimbal lock, shared with inverse kinematics. |
| D6 inequality ownership | Build common D6 rows independently of the direct equality system | Projected full-coordinate modes now retain D6 limits and live property updates without falling back to mode-specific scalar rows; wrench diagnostics include the exact common-row reactions. |
