...
| D6 inequality consolidation | Route native BALL and classified D6 inequalities through common six-axis rows, then remove the legacy angular and scalar axial implementations | Deletes duplicate preparation, warm-start, iteration, and per-column cache state. The production joint column is 72 dwords, down from 84, while common rows retain paired impulses and support limits, speed caps, and friction on the same axes. |
