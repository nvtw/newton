...
"""PhoenX cloth stacking demo (Jitter2 ``Demo20`` port).

Four small cloth grids are spawned 25 cm apart vertically and drop
onto a ground plane.  They settle on top of each other, exercising
the rigid-vs-triangle (RT) and triangle-vs-triangle (TT) contact
paths simultaneously.

The reference scene lives in ``Viewer/Demos/Demo20.cs`` of the
``experimentalsim/jitterphysics2`` C# code base.  The C# version
uses 1 kg per cloth particle, ``RestOffset = 0.04 m``, and 4
substeps × 6 PGS iterations.  We mirror those numbers because the
PGS convergence depends on the cube : per-particle mass ratio --
PhoenX has no mass splitting wired in for cloth, so heavier
particles keep the per-iteration impulse in the regime where the
solver converges cleanly.

Run::

    python -m newton._src.solvers.phoenx.examples.example_cloth_stack
"""
...
class Example:
    """Stack of cloth sheets onto a static cube on a ground plane."""
    ...
    def __init__(
        ...
        # C# Demo20 settings.
        self.sim_substeps = 4
        self.solver_iterations = 6

        self.num_sheets = num_sheets
        self.cloth_dim = cloth_dim
        self.cell = cell
        # Per-particle mass = 1 kg matches the C# Demo20
        # ``InverseMass = 1`` setting and keeps the cube : per-
        # cloth-particle mass ratio in the regime where PGS
        # converges in a few iterations.  Lighter particles need
        # mass splitting which PhoenX hasn't enabled for cloth yet.
        self.particle_mass = 1.0
        self.cloth_thickness = 0.04
        ...
        # ---- Build the model -----------------------------------------
        # Ground plane at z=0 catches the cloth.  Mirrors C# Demo20
        # which has no rigid cube -- just cloth sheets falling onto
        # the playground's ground.
        builder = newton.ModelBuilder()
        builder.add_ground_plane(height=0.0)
        self.cube_body = -1  # no rigid body in this scene

        # Stack the cloth sheets above the ground.
        sheet_w = self.cloth_dim * self.cell  # cloth side length
        sheet_origin_xy = (-0.5 * sheet_w, -0.5 * sheet_w)
        for k in range(self.num_sheets):
            sheet_z = 1.0 + k * 0.25
        ...
        # FK so the cube body shows up in body_q.
        ...
        self.world = PhoenXWorld(
            ...
            rigid_contact_max=max(4096, 8 * int(self.model.tri_count)),
        ...
        self.viewer.set_camera(pos=wp.vec3(8.0, -8.0, 4.0), pitch=-15.0, yaw=135.0)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--num-sheets", type=int, default=4, help="Number of stacked cloth sheets")
    parser.add_argument("--cloth-dim", type=int, default=10, help="Cells per side per sheet")
    viewer, args = newton.examples.init(parser)
    example = Example(viewer, args=args, num_sheets=args.num_sheets, cloth_dim=args.cloth_dim)
