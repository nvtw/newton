...
#
# A 100-link, maximal-coordinate prismatic mechanism used to stress PhoenX's
# mechanism-wide direct equality and implicit-drive solver. The links are
# ordinary Newton bodies and joints. ModelBuilder's required articulation annotation records
...
def _local_joint_frame(
    ...
    """Express a world-aligned slider frame in a body's local frame."""
    inverse_orientation = (
        -body_orientation[0],
        -body_orientation[1],
        -body_orientation[2],
...
def _build_model(num_links: int = NUM_LINKS) -> newton.Model:
    """Build one full-coordinate connected slider mechanism."""
...
class Example:
    """Simulate a long full-coordinate prismatic mechanism with direct equalities."""
    ...
    def __init__(self, viewer, args):
        ...
        if direct is None or not direct.enabled:
            raise RuntimeError("slider mechanism was not assigned to the direct equality solver")
        ...
        if not self.solver.world._joint_pgs_all_disabled:
            raise RuntimeError("bilateral slider or drive rows unexpectedly remain in PGS")
    ...
    def test_final(self) -> None:
        ...
        assert np.isfinite(body_q).all(), "slider chain produced non-finite poses"
        assert np.isfinite(body_qd).all(), "slider chain produced non-finite velocities"

        tip_sag, max_sag = self._tip_sag()
        chain_length = NUM_LINKS * _link_layout()[0]
        print(
            f"[direct_slider_chain] tip_sag={tip_sag * 1.0e3:.3f} mm "
        ...
        assert abs(tip_sag) < maximum_sag, (
            f"direct slider-chain tip error is {tip_sag * 1.0e3:.3f} mm "
            f"({tip_sag / chain_length * 100.0:.3f}% of chain length)"
        )
        assert max_sag < maximum_sag, "direct slider chain bent beyond 1.5% of its length"
