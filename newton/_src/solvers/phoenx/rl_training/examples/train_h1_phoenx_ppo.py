...
"""Train Unitree H1 on flat terrain with PhoenX and Warp-only PPO.

The complete rollout and PPO update run in one CUDA graph. This executable
uses only public Newton APIs and has no IsaacLab runtime dependency.

Example::

    python newton/_src/solvers/phoenx/rl_training/examples/train_h1_phoenx_ppo.py \
        --device cuda:0
"""
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not device.is_cuda or not wp.is_mempool_enabled(device):
        raise RuntimeError("H1 training requires a CUDA device with Warp memory pooling enabled")

    env = rl.EnvH1PhoenX(
        rl.ConfigEnvH1PhoenX(
    ...
    if args.resume_checkpoint is None:
        ...
        if trainer.obs_dim != env.obs_dim or trainer.action_dim != env.action_dim:
            raise ValueError("Checkpoint dimensions do not match H1")
    ...
    for local_iteration in range(int(args.iterations)):
        ...
        if should_log:
            ...
            print(
                ...
                f"tracking={success:.3f} sps={completed_samples / elapsed:,.0f}"
            )
        if (
