...
def run_case(conservative, reverse):
    """Exercise physical activation using cached receding geometric witnesses."""
    ...
    pipeline = newton.CollisionPipeline(
        ...
        speculative_contact_velocity_filter=not conservative,
...
@unittest.skipUnless(wp.is_cuda_available(), "Texture SDF contacts require CUDA")
class TestGeometricMeshActivation(unittest.TestCase):
    def test_dormant_candidates_do_not_apply_impulses(self):
        """Keep receding geometry dormant without changing velocity or momentum."""
        result = run_case(True, False)
        self.assertGreater(result["contacts"], 0)

    def test_internal_reversal_stops_without_added_contact_energy(self):
        """Activate cached witnesses after reversal and conserve all six momenta."""
        baseline = run_case(False, True)
        result = run_case(True, True)
        self.assertEqual(baseline["contacts"], 0)
        self.assertLess(baseline["history"][-1]["physical_axis_gap"], -0.001)
        self.assertGreater(result["contacts"], 0)
        self.assertGreater(min(row["box_sat_separation"] for row in result["history"]), -1e-5)
