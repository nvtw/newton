...
def run(fast, prismatic, split):
    ...
    kernels.joint_constraint_prepare_inequality = (
        joint.joint_constraint_prepare_inequality
        if fast
        else getattr(joint, "_joint_constraint_prepare_inequality_full", joint.joint_constraint_prepare_inequality)
    )
    ...
    for frame in range(10):
        ...
        common_state = (
            ...
            solver.world.constraints.d6.revolution_counter.numpy(),
            solver.world.constraints.d6.previous_angle.numpy(),
        )
        snapshots.append(
            (
                state.body_q.numpy(),
...
class TestInactivePrepare(unittest.TestCase):
    def test_limit_crossing_and_property_transitions(self):
        ...
        try:
            ...
            kernels.joint_constraint_prepare_inequality = joint.joint_constraint_prepare_inequality
