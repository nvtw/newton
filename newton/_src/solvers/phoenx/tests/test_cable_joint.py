...
class TestCableBendStiffness(unittest.TestCase):
    ...
    cable rows are positional (anchor-2 tangent block) but produce the
...
# independent in the cable block-GS solve. Pure-bend deflection should
...
class TestCableDecoupling(unittest.TestCase):
    """Bend and twist are independent rows in the cable block-Gauss-
    Seidel solve."""
...
class TestCableAnalytical(unittest.TestCase):
    ...
    def test_undamped_period_within_5pct(self) -> None:
        ...
        cable prepare's positional rescale ``k_pos = k_user /
    ...
    def test_underdamped_log_decrement_recovers_zeta(self) -> None:
        ...
        first-order discretisation artefact, not cable-specific). Halving
    ...
    def test_high_damping_settles_within_solver_iterations(self) -> None:
        ...
        Both cable and CABLE use the combined :func:`pd_coefficients`
    ...
    def test_large_angle_stable_at_60deg(self) -> None:
        """cable uses the *positional* anchor-2 / anchor-3 rows rather
        ...
        Failure mode: large-angle cable blows up or its period drifts
        ...
        T_meas = 2.0 * (crossings[1] - crossings[0]) * dt
        # cable's positional spring loses force at large angles
        ...
        self.assertLess(
            ...
            0.30,
            msg=(
                f"large-angle cable period drift {rel * 100:.1f}% > 30%; "
...
# ---------------------------------------------------------------------------
#
# The bend rows live in the cable anchor-2 tangent block (2 positional
...
# ``_build_cable_pendulum``) so a pre-rotation about ``+x`` is pure
...
class TestCableTwistAnalytical(unittest.TestCase):
    """Quantitative comparison of cable twist vs the closed-form damped
    harmonic oscillator. Mirror of :class:`TestCableAnalytical` with
    ...
    def test_undamped_period_within_5pct(self) -> None:
        ...
        I}`. A drift here means the cable prepare's twist gain
    ...
    def test_underdamped_log_decrement_recovers_zeta(self) -> None:
        ...
        :class:`TestCableAnalytical`); empirically both axes land at
    ...
    def test_overdamped_settling_time_constant(self) -> None:
        ...
        self.assertEqual(
            crossings,
            0,
            msg=(f"twist overdamped trajectory should not cross zero; got {crossings} crossings"),
        )
...
class TestCableDegenerate(unittest.TestCase):
    """Edge cases."""

    def test_zero_stiffness_does_not_blow_up(self) -> None:
        """With zero stiffness and zero damping on both axes, cable
        ...
        active block. cable's anchor-2 / anchor-3 prepare *also*
        ...
        This is an internal cable behaviour gap surfaced by the
        validation suite, not a stability failure. The assertion
        below pins that pose / velocity stay bounded and finite,
        leaving the "CABLE[0,0] == ball-socket" guarantee out of
        scope until the cable kernel grows the same short-circuit
        plumbing CABLE has."""
        omega0 = (1.0, -0.5, 0.3)
        world = _build_cable_pendulum(
            wp.get_preferred_device(),
        ...
        self.assertTrue(np.isfinite(omega).all(), msg=f"CABLE[0,0] omega non-finite: {omega}")
        self.assertTrue(np.isfinite(q).all(), msg=f"CABLE[0,0] orientation non-finite: {q}")
        self.assertTrue(np.isfinite(pos).all(), msg=f"CABLE[0,0] position non-finite: {pos}")
        ...
        self.assertLess(
            ...
            0.05,
            msg=f"CABLE[0,0] anchor-1 lock leaked: |com|={np.linalg.norm(pos):.4f} m",
        )
...
class TestCableGainConversion(unittest.TestCase):
    ...
    def _measure_period(self, rest_length: float, k: float) -> tuple[float, float]:
        ...
        crossings = _zero_crossings(angles)
        if len(crossings) < 2:
            self.fail(f"rest_length={rest_length:.3g}: <2 zero crossings; period unmeasurable (got {len(crossings)})")
        if len(crossings) >= 3:
            T_meas = (crossings[2] - crossings[0]) * dt
        else:
            T_meas = 2.0 * (crossings[1] - crossings[0]) * dt
...
def _build_cable_cantilever_chain(
    device,
    *,
    n_segments: int,
    pitch: float,
    ...
    built out of ``n_segments`` capsules linked by cable joints.

    Layout matches ``example_cable_chain.py``: the leftmost capsule
    is a static anchor at ``x = 0``, then ``n_segments - 1`` dynamic
    capsules tile along +x at spacing ``pitch``. Gravity points in
    -z. Each cable joint shares an attachment at the midpoint between
...
class TestCableCantileverStatic(unittest.TestCase):
    """Joint-segmented Euler-Bernoulli static deflection.

    A cantilever of ``N - 1`` dynamic capsules attached to a static
    anchor by cable joints (each with bend stiffness ``k``) carries a
    ...
    def test_tip_angle_matches_euler_bernoulli(self) -> None:
        ...
        world, bodies = _build_cable_cantilever_chain(
            wp.get_preferred_device(),
        ...
        self.assertLess(
            ...
            msg=(
                ...
                f"Catches lever-arm scale errors in cable's k_pos = k_user / rest_length^2 rescale."
            ),
        )
...
class TestCableChainConservation(unittest.TestCase):
    """In a static-equilibrium cable cantilever the reaction torque
    ...
    *body orientations* at static equilibrium -- ``theta_j = M_j /
    ...
    def test_joint_angle_drops_decrease_root_to_tip(self) -> None:
        ...
        # Bend axis: about world +y (gravity is -z, cable axis is +x).
        y_axis = np.array([0.0, 1.0, 0.0])
