...
"""Newton-side ``JointType.CABLE`` -> PhoenX ``JointMode.BEAM`` adapter
tests (opt-in routing via ``SolverPhoenX(use_beam_for_cable=True)``).

The PhoenX BEAM constraint itself is exercised analytically in
``test_beam_joint.py`` -- this file checks the *adapter glue*: that
:meth:`ModelBuilder.add_joint_cable` survives ``model.finalize()`` and
lands on PhoenX's beam mode (not cable mode) when the opt-in flag is
set, with the right anchor / stiffness / damping wiring.

PhoenX has no axial-length compliance, so Newton's stretch DoF is
treated as rigid (same as the CABLE adapter path). The tests assert
(a) the rigid ball-socket holds the parent and child attachments
coincident under load and (b) the user-supplied isotropic bend
stiffness produces a measurable restoring torque on the rotation
between the two bodies, scaling correctly with the bend gain.
"""
...
def _two_body_cable_world(
    ...
    * Body 2: 1 kg cube hanging from body 1 via a ``CABLE`` joint
    ...
    ``use_beam_for_cable`` is forwarded to :class:`SolverPhoenX` so
    the adapter routes the CABLE joint to BEAM (or CABLE) on the
    PhoenX side.
    """
    ...
    solver = newton.solvers.SolverPhoenX(
        ...
        use_beam_for_cable=use_beam_for_cable,
    )
    return model, solver
...
def _step_n(model: newton.Model, solver, frames: int, dt: float) -> tuple[np.ndarray, np.ndarray]:
    ...
    The capture pattern is adapted from
    ``newton/examples/_basic_setup.py`` and
    ``newton/_src/solvers/phoenx/tests/test_cable_joint_newton.py``;
    differences are only in collecting ``peak_disp`` per replay so a
    diverging cable scene still fails fast.
    """
...
@unittest.skipUnless(
    ...
    "PhoenX cable->beam tests run on CUDA only (graph-capture path).",
)
class TestNewtonCableBeamAdapter(unittest.TestCase):
    """Verify ``add_joint_cable`` -> PhoenX BEAM wiring under
    ``SolverPhoenX(use_beam_for_cable=True)``.
    """

    def test_beam_constructs_without_error(self) -> None:
        """``add_joint_cable`` -> BEAM must build a valid solver
        column. Mirrors :meth:`test_cable_joint_newton.test_cable_constructs_without_error`."""
        model, solver = _two_body_cable_world(
            bend_stiffness=10.0, bend_damping=0.5, use_beam_for_cable=True
        )
    ...
    def test_phoenx_beam_mode_id_set(self) -> None:
        """The descriptor for the Newton cable joint must end up
        tagged as PhoenX :data:`JOINT_MODE_BEAM` (not CABLE / FIXED /
        etc.) when ``use_beam_for_cable=True``. With the flag off
        the same joint must route to :data:`JOINT_MODE_CABLE`,
        confirming the routing is opt-in only."""
        _model, solver_beam = _two_body_cable_world(
            bend_stiffness=10.0, bend_damping=0.5, use_beam_for_cable=True
        )
        modes_beam = solver_beam._adbs.joint_mode.numpy()
        self.assertIn(int(JOINT_MODE_BEAM), modes_beam.tolist())
        self.assertNotIn(int(JOINT_MODE_CABLE), modes_beam.tolist())

        _model, solver_cable = _two_body_cable_world(
            bend_stiffness=10.0, bend_damping=0.5, use_beam_for_cable=False
        )
        modes_cable = solver_cable._adbs.joint_mode.numpy()
        self.assertIn(int(JOINT_MODE_CABLE), modes_cable.tolist())
        self.assertNotIn(int(JOINT_MODE_BEAM), modes_cable.tolist())

    def test_beam_holds_attachment_under_gravity(self) -> None:
        ...
        times. This is the BEAM equivalent of the CABLE adapter's
        rigid-attachment regression."""
        model, solver = _two_body_cable_world(
            bend_stiffness=10.0, bend_damping=2.0, use_beam_for_cable=True
        )
        ...
        np.testing.assert_allclose(
            ...
            anchor_pos,
            atol=5.0e-3,
            err_msg=f"BEAM attachment slipped: bob_attach={bob_attach_world}, anchor={anchor_pos}",
        )

    def test_beam_bend_gain_resists_gravity(self) -> None:
        """Bend gain wiring smoke test: the BEAM bend spring must
        ...
        BEAM bend axes (with default ``-Z`` gravity the cable axis is
        ...
        stiff must produce a *significantly smaller* tilt. The
        absolute equilibrium angle depends on BEAM's positional
        ``sin(theta)`` softness vs. the rotational ``theta`` model
        (validated quantitatively in
        :class:`TestBeamCantileverStatic` using a multi-segment
        chain), so this Newton-side test only checks the *ordering*:
        gain plumbed through -> stiffer spring -> smaller tilt.
        """
        g = 9.81
        ...
        self.assertLess(
            ...
            0.2 * tilt_soft,
            msg=(
                f"BEAM bend gain wiring: stiff k=2000 tilt={math.degrees(tilt_stiff):.3f} deg, "
