...
@wp.kernel(enable_backward=False)
def _solve_family_split_kernel(
    row_ids_by_family: wp.array[wp.int32],
    ...
    while epoch < iterations:
        color = color_begin
        while color < color_end:
            family_base = color * wp.int32(5)

            start = color_family_starts[family_base]
            end = color_family_starts[family_base + wp.int32(1)]
            cursor = start + local_tid
            while cursor < end:
                row = row_ids_by_family[cursor]
                va = v_a[row]
                wa = w_a[row]
                vb = v_b[row]
                wb = w_b[row]
            ...
            family_id = wp.int32(1)
            while family_id < wp.int32(4):
                linear = wp.vec3f(wp.float32(0.0), wp.float32(0.0), wp.float32(0.0))
                cross = wp.vec3f(wp.float32(0.0), wp.float32(0.0), wp.float32(0.0))
                angular = wp.vec3f(wp.float32(0.0), wp.float32(0.0), wp.float32(0.0))
                if family_id == wp.int32(1):
                    linear = wp.vec3f(wp.float32(1.0), wp.float32(1.0), wp.float32(1.0))
                    cross = wp.vec3f(wp.float32(1.0), wp.float32(1.0), wp.float32(1.0))
                elif family_id == wp.int32(2):
                    angular = wp.vec3f(wp.float32(1.0), wp.float32(1.0), wp.float32(1.0))
                else:
                    linear = wp.vec3f(wp.float32(1.0), wp.float32(0.0), wp.float32(0.0))
                    angular = wp.vec3f(wp.float32(0.0), wp.float32(1.0), wp.float32(1.0))

                start = color_family_starts[family_base + family_id]
                end = color_family_starts[family_base + family_id + wp.int32(1)]
                cursor = start + local_tid
                while cursor < end:
                    row = row_ids_by_family[cursor]
                    rows = RigidFrameBlock3Mixed()
                    ...
                    out_lambda[row] = mixed_update.lambda_new
                    cursor = cursor + threads_per_world
                family_id = family_id + wp.int32(1)
...
def _row_family_host(
    ...
    if lane < contact_slots:
        return 0
    if lane < contact_slots + point_slots:
        return 1
    if lane < contact_slots + point_slots + angular_slots:
        return 2
    return 3


def _build_family_grouped_schedule(
    row_ids: np.ndarray,
    ...
    grouped: list[int] = []
    color_count = int(color_starts.shape[0]) - 1
    family_starts = np.empty(color_count * 5, dtype=np.int32)
    for color in range(color_count):
        buckets: tuple[list[int], list[int], list[int], list[int]] = ([], [], [], [])
        for row in row_ids[int(color_starts[color]) : int(color_starts[color + 1])]:
            family = _row_family_host(int(row), period, contact_slots, point_slots, angular_slots)
            buckets[family].append(int(row))
        base = color * 5
        for family in range(4):
            family_starts[base + family] = len(grouped)
            grouped.extend(buckets[family])
        family_starts[base + 4] = len(grouped)
    return np.asarray(grouped, dtype=np.int32), family_starts
...
def _run_scene(args: argparse.Namespace, scene: str, device: wp.context.Devicelike) -> None:
    ...
    family_va = _alloc_vec(rows, device)
    family_wa = _alloc_vec(rows, device)
    family_vb = _alloc_vec(rows, device)
    family_wb = _alloc_vec(rows, device)
    family_lambda = _alloc_vec(rows, device)
    ...
    grouped_row_ids_host, color_family_starts_host = _build_family_grouped_schedule(
        schedule.row_ids,
        schedule.color_starts,
        int(args.period),
        contact_slots,
        point_slots,
        angular_slots,
    )
    row_ids_by_family = wp.array(grouped_row_ids_host, dtype=wp.int32, device=device)
    color_family_starts = wp.array(color_family_starts_host, dtype=wp.int32, device=device)
    ...
    family_ms, _ = _bench(family_run, n_runs=args.n_runs, warmup=args.warmup, trials=args.trials, device=device)
    ...
    family_speedup = split_ms / family_ms if family_ms > 0.0 else float("nan")
    print(
        ...
        f"hybrid={hybrid_ms:8.4f}ms family={family_ms:8.4f}ms "
        f"speedups=({mixed_speedup:6.3f}x,{tagged_speedup:6.3f}x,{hybrid_speedup:6.3f}x,{family_speedup:6.3f}x) "
        f"errs=({mixed_err:.6g},{tagged_err:.6g},{hybrid_err:.6g},{family_err:.6g})"
    )
