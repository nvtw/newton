...
newton._src.solvers.jitter.tests.test_run_all`` is much faster than
``newton.tests`` because it skips the rest of the project's test
suite (and the associated solver/example warm-up).

Run as::

    uv run --extra dev -m unittest newton._src.solvers.jitter.tests.test_run_all

or directly::

    uv run --extra dev python -m newton._src.solvers.jitter.tests.test_run_all
"""
...
def load_tests(loader: unittest.TestLoader, standard_tests, pattern):
    ...
    ``newton._src.solvers.jitter.<stem>``; this avoids
