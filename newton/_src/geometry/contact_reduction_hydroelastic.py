...
def create_export_hydroelastic_reduced_contacts_kernel(
    ...
    def export_hydroelastic_reduced_contacts_kernel(
        ...
        for i in range(tid, num_active, total_num_threads):
            ...
            # ``agg_force`` is stored in physical force units after applying
            # ``pressure_func``.  The normal-matching reliability threshold was
            # tuned for the pre-callback depth-volume aggregate.  Contacts store
            # the SDF depth of shape_b, so dividing by shape_b's linear pressure
            # scale restores the old units exactly for the default callback.
            ...
            for idx in range(num_exported):
                ...
                if has_reliable_agg_direction:
                    ...
                    if nbin_entry_idx >= 0 and depth < 0.0:
                        ...
                        nbin_agg_mag = wp.length(nbin_agg_force)
                        nbin_direction_mag = nbin_agg_mag / _pressure_to_depth_scale(
