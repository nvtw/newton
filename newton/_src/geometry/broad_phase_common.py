...
def check_obb_overlap(
    ...
    d = center_b - center_a
    # Columns of A's rotation
    ax0 = wp.vec3(rot_a[0, 0], rot_a[1, 0], rot_a[2, 0])
    ax1 = wp.vec3(rot_a[0, 1], rot_a[1, 1], rot_a[2, 1])
    ax2 = wp.vec3(rot_a[0, 2], rot_a[1, 2], rot_a[2, 2])
    bx0 = wp.vec3(rot_b[0, 0], rot_b[1, 0], rot_b[2, 0])
    bx1 = wp.vec3(rot_b[0, 1], rot_b[1, 1], rot_b[2, 1])
    bx2 = wp.vec3(rot_b[0, 2], rot_b[1, 2], rot_b[2, 2])
    ...
    # A's 3 face axes
    if wp.abs(da0) > half_a[0] + half_b[0] * a00 + half_b[1] * a01 + half_b[2] * a02:
        return False
    if wp.abs(da1) > half_a[1] + half_b[0] * a10 + half_b[1] * a11 + half_b[2] * a12:
        return False
    if wp.abs(da2) > half_a[2] + half_b[0] * a20 + half_b[1] * a21 + half_b[2] * a22:
        return False
    # B's 3 face axes
    db0 = da0 * c00 + da1 * c10 + da2 * c20
    db1 = da0 * c01 + da1 * c11 + da2 * c21
    db2 = da0 * c02 + da1 * c12 + da2 * c22
    if wp.abs(db0) > half_a[0] * a00 + half_a[1] * a10 + half_a[2] * a20 + half_b[0]:
        return False
    if wp.abs(db1) > half_a[0] * a01 + half_a[1] * a11 + half_a[2] * a21 + half_b[1]:
        return False
    if wp.abs(db2) > half_a[0] * a02 + half_a[1] * a12 + half_a[2] * a22 + half_b[2]:
        return False
    # 9 edge-edge axes
    if wp.abs(da2 * c10 - da1 * c20) > half_a[1] * a20 + half_a[2] * a10 + half_b[1] * a02 + half_b[2] * a01:
        return False
    if wp.abs(da2 * c11 - da1 * c21) > half_a[1] * a21 + half_a[2] * a11 + half_b[0] * a02 + half_b[2] * a00:
        return False
    if wp.abs(da2 * c12 - da1 * c22) > half_a[1] * a22 + half_a[2] * a12 + half_b[0] * a01 + half_b[1] * a00:
        return False
    if wp.abs(da0 * c20 - da2 * c00) > half_a[0] * a20 + half_a[2] * a00 + half_b[1] * a12 + half_b[2] * a11:
        return False
    if wp.abs(da0 * c21 - da2 * c01) > half_a[0] * a21 + half_a[2] * a01 + half_b[0] * a12 + half_b[2] * a10:
        return False
    if wp.abs(da0 * c22 - da2 * c02) > half_a[0] * a22 + half_a[2] * a02 + half_b[0] * a11 + half_b[1] * a10:
        return False
    if wp.abs(da1 * c00 - da0 * c10) > half_a[0] * a10 + half_a[1] * a00 + half_b[1] * a22 + half_b[2] * a21:
        return False
    if wp.abs(da1 * c01 - da0 * c11) > half_a[0] * a11 + half_a[1] * a01 + half_b[0] * a22 + half_b[2] * a20:
        return False
    if wp.abs(da1 * c02 - da0 * c12) > half_a[0] * a12 + half_a[1] * a02 + half_b[0] * a21 + half_b[1] * a20:
        return False
    return True
