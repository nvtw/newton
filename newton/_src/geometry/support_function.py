...
def extract_shape_data(
    ...
    shape_data: wp.array(dtype=wp.vec4),  # scale (xyz), gap (w) or other data
    ...
    Args:
        ...
        shape_data: Shape data (vec4 - scale xyz, gap w)
        shape_source: Source pointers (mesh IDs etc.)

    Returns:
        tuple: (position, orientation, shape_data, scale, gap)
    """
    ...
    # Extract scale and gap from shape_data
    # Assuming shape_data stores scale in xyz and gap in w
    ...
    return position, orientation, result, scale, gap
