...
@wp.func
def support_map_gjk(geom: GenericShapeData, direction: wp.vec3, data_provider: SupportMapDataProvider) -> wp.vec3:
    """Support map for the GJK/MPR kernel: all convex types, no PLANE/TRIANGLE.

    The GJK/MPR kernel never processes plane, triangle, or triangle-prism shapes
    (those are routed to specialized pipelines). Omitting those branches reduces
    compiled code size and improves GPU instruction cache utilization.
    """
    eps = 1.0e-12
    result = wp.vec3(0.0, 0.0, 0.0)
