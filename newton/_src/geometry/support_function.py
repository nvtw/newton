...
def extract_shape_data(
    ...
    shape_data: wp.array(dtype=wp.vec4),  # scale (xyz), margin (w) or other data
    ...
    Args:
        ...
        shape_data: Shape data (vec4 - scale xyz, margin w)
        shape_source: Source pointers (mesh IDs etc.)

    Returns:
        tuple: (position, orientation, shape_data, scale, margin)
    """
    ...
    # Extract scale and margin from shape_data
    # Assuming shape_data stores scale in xyz and margin in w
    ...
    return position, orientation, result, scale, margin
