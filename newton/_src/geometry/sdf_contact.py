...
def _create_get_edge_from_mesh_func(use_precomputed_edge_data: bool):
    ...
    def get_edge_from_mesh_func(
        ...
        if wp.static(use_precomputed_edge_data):
            return get_edge_from_mesh_precomputed(mesh_edge_centers, mesh_edge_halves, edge_range, X_mesh_ws, edge_idx)
...
def compute_mesh_mesh_edge_counts(
    ...
    prefix. Values beyond ``pair_count`` cannot affect the consumed prefix of
    the subsequent exclusive scan.
    """
...
def compute_block_counts_from_weights(
    ...
    ``weight_per_block`` threshold, then assigns each pair a block count
    proportional to its weight. Only the active prefix is consumed from the
    subsequent exclusive scan.
    """
...
def compute_mesh_mesh_block_offsets_scan(
    ...
    Runs a three-stage parallel pipeline: per-pair edge counts and their scalar
    total → adaptive block counts → exclusive scan into ``block_offsets``.
    """
