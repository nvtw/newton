...
class ContactData:
    ...
    Attributes:
        ...
        gap_a: Collision gap (outward offset) for shape A [m]
        gap_b: Collision gap (outward offset) for shape B [m]
        shape_a: Index of the first shape in the collision pair
        shape_b: Index of the second shape in the collision pair
        pair_margin: Sum of per-shape margins (margin_a + margin_b) for contact threshold [m]
...
def contact_passes_margin_check(
    ...
    """
    Check if a contact passes the threshold and should be written.
    ...
    Returns:
        True if the contact distance is within the contact threshold, False otherwise
    """
