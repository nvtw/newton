...
class NarrowPhase:
    def __init__(
        ...
        Args:
            ...
            has_meshes: Whether the scene contains mesh shapes. When False, mesh-related kernels
                (mesh-plane, mesh-mesh, mesh-triangle overlap/contacts) are skipped for better performance.
                Defaults to True.
        """
    ...
    def launch_custom_write(
        ...
        # Launch mesh-related kernels only if scene contains meshes
        if self.has_meshes:
        ...
        # Launch mesh triangle contact processing kernel (only if scene has meshes)
        if self.has_meshes:
            if self.reduce_contacts:
