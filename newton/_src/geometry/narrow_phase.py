...
def write_contact_simple(
    ...
    contact_threshold = pair_gap + contact_data.pair_margin
...
def create_narrow_phase_primitive_kernel(writer_func: Any):
    ...
    def narrow_phase_primitive_kernel(
        ...
        for t in range(tid, num_work_items, total_num_threads):
            ...
            margin_a = shape_margin[shape_a]
            margin_b = shape_margin[shape_b]
            ...
            if num_contacts > 0:
                ...
                contact_data.gap_a = gap_a
                contact_data.gap_b = gap_b
                contact_data.shape_a = shape_a
                contact_data.shape_b = shape_b
                contact_data.pair_margin = margin
...
def create_narrow_phase_kernel_gjk_mpr(external_aabb: bool, writer_func: Any):
    ...
    def narrow_phase_kernel_gjk_mpr(
        ...
        for t in range(tid, num_work_items, total_num_threads):
            ...
            # Extract shape data
            pos_a, quat_a, shape_data_a, scale_a, gap_a = extract_shape_data(
                shape_a, shape_transform, shape_types, shape_data, shape_source
            )
            pos_b, quat_b, shape_data_b, scale_b, gap_b = extract_shape_data(
...
def narrow_phase_find_mesh_triangle_overlaps_kernel(
    ...
    shape_transform: wp.array(dtype=wp.transform),
    shape_source: wp.array(dtype=wp.uint64),
    shape_margin: wp.array(dtype=float),  # Per-shape contact margins
    shape_data: wp.array(dtype=wp.vec4),  # Shape data (scale xyz, gap w)
    shape_pairs_mesh: wp.array(dtype=wp.vec2i),
    shape_pairs_mesh_count: wp.array(dtype=int),
    total_num_threads: int,
    # outputs
    ...
    for i in range(tid, num_mesh_pairs, total_num_threads):
        ...
        # Sum margins for consistency with gap summing
        margin_non_mesh = shape_margin[non_mesh_shape]
        margin_mesh = shape_margin[mesh_shape]
...
def create_narrow_phase_process_mesh_triangle_contacts_kernel(writer_func: Any):
    ...
    def narrow_phase_process_mesh_triangle_contacts_kernel(
        ...
        for i in range(tid, num_triangle_pairs, total_num_threads):
            ...
            pos_b, quat_b, shape_data_b, _scale_b, gap_b = extract_shape_data(
                shape_b,
                shape_transform,
                shape_types,
                shape_data,
            ...
            # Extract gap for shape A
            gap_a = shape_data[shape_a][3]
...
def create_narrow_phase_process_mesh_plane_contacts_kernel(
    ...
    def narrow_phase_process_mesh_plane_contacts_kernel(
        ...
        for pair_idx in range(pair_count):
            ...
            # Extract gap values
            gap_mesh = shape_data[mesh_shape][3]
            gap_plane = shape_data[plane_shape][3]
            total_gap = gap_mesh + gap_plane

            # Use per-shape contact margin for contact detection
            # Sum margins for consistency with gap summing
            margin_mesh = shape_margin[mesh_shape]
            margin_plane = shape_margin[plane_shape]
            ...
            for vertex_idx in range(tid, num_vertices, total_num_threads):
                ...
                if distance < margin + total_gap:
                    ...
                    contact_data.gap_a = gap_mesh
                    contact_data.gap_b = gap_plane
...
def heightfield_midphase_kernel(
    ...
    for i in range(tid, num_pairs, total_num_threads):
        ...
        margin = shape_margin[hfield_shape] + shape_margin[other_shape]
...
def create_heightfield_triangle_contacts_kernel(writer_func: Any):
    ...
    def heightfield_triangle_contacts_kernel(
        ...
        for i in range(tid, num_cell_pairs, total_num_threads):
            ...
            gap_a = shape_data[hfield_shape][3]
            margin_a = shape_margin[hfield_shape]
            margin_b = shape_margin[convex_shape]
            margin = margin_a + margin_b
...
class NarrowPhase:
    ...
    def launch_custom_write(
        ...
        *,
        ...
        shape_data: wp.array(dtype=wp.vec4, ndim=1),  # Shape data (scale xyz, gap w)
        ...
        shape_margin: wp.array(dtype=wp.float32, ndim=1),  # per-shape contact margin
        ...
        Args:
            ...
            shape_data: Array of vec4 containing scale (xyz) and gap (w) for each shape
            ...
            shape_margin: Array of contact margins for each shape
