...
# Left to its own devices the compiler allocates all 255 registers for the
# generation kernel and still spills, which caps residency at 8 warps per SM.
# Asking for 12 resident blocks bounds it to 168 registers, which it meets with
# almost no spilling, and the extra warps hide the corner-sampling latency
# (measured -14% kernel time at 20 nut/bolt worlds and -45% at 80).
HYDRO_GENERATE_MIN_BLOCKS_PER_SM = 12
# The dense first refinement level samples eight children per thread; bounding
# it to four resident blocks trades a few registers for twice the warps.
HYDRO_COUNT_MIN_BLOCKS_PER_SM = 4
# Minimum thread count for the light grid-stride kernels (scatters, reduce,
# export, decode). Their per-step work is a few items per narrow-band tile, so
# launching the full ``Config.grid_size`` (a quarter million threads) for a
# small scene makes grid ramp-up dominate, while a fixed small grid starves
# large scenes. The launch is sized from the traversed narrow-band tiles with
# this floor.
HYDRO_COMPACT_GRID_MIN = 16384
HYDRO_COMPACT_THREADS_PER_TILE = 2
...
class HydroelasticSDF:
    ...
    def __init__(
        ...
        with wp.ScopedDevice(device):
            ...
            self.compact_grid_size = min(
                self.grid_size,
                max(HYDRO_COMPACT_GRID_MIN, HYDRO_COMPACT_THREADS_PER_TILE * self.total_num_active_tiles),
            )
    ...
    def _broadphase_sdfs(
        ...
        wp.launch(
            ...
            dim=[self.compact_grid_size],
            inputs=[
                self.compact_grid_size,
                self.num_blocks_per_pair,
    ...
    def _reduce_decode_contacts(
        ...
        self.contact_reduction.reduce(
            ...
            grid_size=self.compact_grid_size,
        )
...
def create_count_iso_voxels_block_kernel(pressure_func: Any, integer_center: bool, paired_samples: bool):
    ...
    @wp.kernel(enable_backward=False, launch_bounds=(HYDRO_COUNT_BLOCK_DIM, HYDRO_COUNT_MIN_BLOCKS_PER_SM))
...
def get_generate_contacts_kernel(
    ...
    @wp.kernel(enable_backward=False, launch_bounds=(HYDRO_GENERATE_BLOCK_DIM, HYDRO_GENERATE_MIN_BLOCKS_PER_SM))
