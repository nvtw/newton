...
class ViewerBase:
    ...
    def log_shapes(
        ...
        Args:
            ...
            geo_gap: Optional gap (used for hashing consistency)
    ...
    def log_geo(
        ...
        if geo_type in (newton.GeoType.MESH, newton.GeoType.CONVEX_MESH):
            ...
            if not geo_is_solid:
                indices, points = solidify_mesh(geo_src.indices, geo_src.vertices, geo_gap)
            else:
    ...
    def _hash_geometry(self, geo_type: int, geo_scale, gap: float, is_solid: bool, geo_src=None) -> int:
        return hash((int(geo_type), geo_src, *geo_scale, float(gap), bool(is_solid)))
    ...
    def _populate_shapes(self):
        ...
        shape_geo_gap = self.model.shape_gap.numpy()
    ...
    def _populate_inertia_boxes(self):
        ...
        is_solid = True
        geo_src = None
        geo_args = (newton.GeoType.BOX, scale, gap, is_solid, geo_src)
