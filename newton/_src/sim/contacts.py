...
class Contacts:
    ...
    This class manages buffers for contact data such as positions, normals, gaps, and shape indices
    ...
    def __init__(
        ...
        with wp.ScopedDevice(device):
            ...
            self.rigid_contact_gap0 = wp.zeros(rigid_contact_max, dtype=wp.float32)
            """Contact gap for shape 0 [m], shape (rigid_contact_max,), dtype float."""
            self.rigid_contact_gap1 = wp.zeros(rigid_contact_max, dtype=wp.float32)
            """Contact gap for shape 1 [m], shape (rigid_contact_max,), dtype float."""
