...
class ModelBuilder:
    ...
    class ShapeConfig:
        ...
        """SDF AABB padding [m] for primitive texture SDFs.

        Falls back to :attr:`gap` when ``None``. Distinct from :attr:`gap`
        (broad-phase inflation) and :attr:`margin` (contact-surface inflation).
        Rejected on ``MESH`` and ``CONVEX_MESH`` shapes; pass ``margin`` to
        :meth:`~newton.geometry.Mesh.build_sdf` before
        :meth:`ModelBuilder.add_shape_mesh` instead.
        """
        ...
        *,
        ...
        Args:
            ...
            skip_shape_contact_pairs: If True, skips the O(n^2) precomputed explicit shape-contact pair list.
                Use only with collision pipelines that do not consume ``Model.shape_contact_pairs``, such as SAP
                or NXN broad phases. Default is False.
