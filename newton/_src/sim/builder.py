...
class ModelBuilder:
    ...
    class ShapeConfig:
        ...
        their gaps are summed (gap_a + gap_b) to determine the total separation."""
        margin: float | None = None
        """The contact margin for collision detection. If None, uses builder.rigid_contact_margin as default.
        AABBs are expanded by (gap + margin) per shape for broad phase. Each shape should have
        (gap + margin) > 0 for reliable collision detection."""
        is_solid: bool = True
    ...
    def add_shape(
        ...
        self.shape_margin.append(
            cfg.margin if cfg.margin is not None else self.rigid_contact_margin
        )
        ...
        if not is_static and cfg.density > 0.0 and body >= 0 and not self.body_lock_inertia[body]:
            (m, c, I) = compute_inertia_shape(type, scale, src, cfg.density, cfg.is_solid, cfg.gap)
    ...
    def _validate_shapes(self) -> bool:
        """Validate shape gap+margin for stable broad phase detection.

        AABBs are expanded by (gap + margin) per shape. For reliable broad phase detection,
        each colliding shape should have (gap + margin) > 0 so that surfaces produce
        overlapping AABBs when within contact threshold.

        This check only considers shapes that participate in collisions (with the
        `COLLIDE_SHAPES` or `COLLIDE_PARTICLES` flag).

        Warns:
            UserWarning: If any colliding shape has (gap + margin) <= 0.

        Returns:
            bool: True if no shapes with (gap + margin) <= 0, False otherwise.
        """
        collision_flags_mask = ShapeFlags.COLLIDE_SHAPES | ShapeFlags.COLLIDE_PARTICLES
        shapes_with_bad_expansion = []
        for i in range(self.shape_count):
            ...
            expansion = gap + margin
            if expansion <= 0:
                shapes_with_bad_expansion.append(
                    f"{self.shape_label[i] or f'shape_{i}'} (gap={gap:.6g}, margin={margin:.6g}, expansion={expansion:.6g})"
                )
        if shapes_with_bad_expansion:
            example_shapes = shapes_with_bad_expansion[:5]
            warnings.warn(
                f"Found {len(shapes_with_bad_expansion)} shape(s) with (gap + margin) <= 0. "
                f"This can cause missed collisions in broad phase since AABBs are expanded by gap + margin. "
                f"Set gap + margin > 0 for each shape. "
                f"Affected shapes: {example_shapes}" + ("..." if len(shapes_with_bad_expansion) > 5 else ""),
                stacklevel=2,
            )
        return len(shapes_with_bad_expansion) == 0
