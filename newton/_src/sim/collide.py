...
def write_contact(
    ...
    # Use per-shape contact margins (sum of both shapes, consistent with margin)
    margin_a = writer_data.shape_gap[contact_data.shape_a]
    margin_b = writer_data.shape_gap[contact_data.shape_b]
...
def compute_shape_aabbs(
    ...
    AABBs are enlarged by per-shape ``margin + gap`` for conservative broad-phase detection.
    """
    shape_id = wp.tid()
    ...
    # Enlarge AABB by per-shape margin + gap for contact detection
    margin = shape_margin[shape_id]
    gap = shape_gap[shape_id]
    aabb_expansion = margin + gap
    margin_vec = wp.vec3(aabb_expansion, aabb_expansion, aabb_expansion)
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            ...
            soft_gap (float, optional): Margin for soft contact generation. Defaults to 0.01.
    ...
    def collide(
        ...
        *,
        ...
        Args:
            ...
            soft_gap: Margin for soft contact generation. If None, uses the value from construction.
        ...
        soft_gap = soft_gap if soft_gap is not None else self.soft_gap
