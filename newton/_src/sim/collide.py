...
class CollisionPipeline:
        ...
        *,
        ...
        if using_expert_components:
            ...
            if self.broad_phase_mode == "explicit":
                ...
                self.broad_phase = BroadPhaseSAP(
                    shape_world, shape_flags=shape_flags, sort_type="auto", device=device, **bp_filter_kwargs
                )
                self.shape_pairs_filtered = None
                self.shape_pairs_max = _resolve_shape_pairs_max(model, shape_pairs_max, self.extra_shape_count)
                self.shape_pairs_excluded = self._build_excluded_pairs(model)
            ...
            split_gjk_mpr = (
                device.is_cuda and has_generic_convex_pairs and generic_convex_pair_work_estimate >= split_threshold
            )
        ...
        if deterministic:
            ...
            self._contact_sorter = ContactSorter(
                ...
                source_shape_index_bits=self._contact_sort_shape_index_bits,
                source_subkey_bits=self._contact_sort_sub_key_bits,
    ...
    def collide(
        ...
        *,
        ...
        self._generate_soft_contacts(state, contacts, soft_contact_gap)
        contacts._contact_matching_mode = self.contact_matching
    ...
    def _run_broad_phase(
        ...
        *,
        ...
        """
        shape_body = self.unified_shape_body if shape_count > self.model.shape_count else self.model.shape_body
        broad_phase_speculative_kwargs = (
            {"shape_displacement": shape_displacement} if shape_displacement is not None else {}
        )
        if isinstance(self.broad_phase, BroadPhaseAllPairs):
            self.broad_phase.launch(
                ...
                body_flags=self.model.body_flags,
    ...
    def _generate_soft_contacts(self, state: State, contacts: Contacts, soft_contact_gap: float) -> None:
