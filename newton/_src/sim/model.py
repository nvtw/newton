...
class Model:
    ...
    def __init__(self, device: Devicelike | None = None):
        ...
        """Per-shape geometry handle for Warp kernels, shape [shape_count], uint64.
        Valid mesh handle for ``MESH`` / ``CONVEX_MESH`` (see :meth:`newton.Mesh.finalize`);
        ``0`` for ``HFIELD`` (heightfield collision data lives in
        :attr:`shape_heightfield_index` / :attr:`heightfield_data` /
        :attr:`heightfield_elevations`)."""
