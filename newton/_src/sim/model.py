...
class Model:
    ...
    def __init__(self, device: Devicelike | None = None):
        ...
        """Shape margin [m], shape [shape_count], float."""
        self.shape_source = []
        ...
        Computed from base geometry only (excludes margin - margin is added during contact
    ...
    def contacts(
        ...
        Note:
            Rigid contact margins are controlled per-shape via :attr:`Model.shape_gap`, which is populated
            from ``ShapeConfig.gap`` during model building. If a shape doesn't specify a contact margin,
            it defaults to ``builder.rigid_contact_gap``. To adjust contact margins, set them before calling
