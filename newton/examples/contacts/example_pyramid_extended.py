...
class BoxRepresentation(Enum):
    """How the pyramid cubes are represented for collision."""
...
BOX_REPRESENTATION = BoxRepresentation.CONVEX_HULL
SDF_RESOLUTION = 128
...
def _build_cube_mesh(half_extent: float) -> newton.Mesh:
    """Build a unit cube mesh scaled to ``half_extent`` (8 verts, 12 tris)."""
    h = half_extent
    vertices = np.array(
        [
            [-h, -h, -h],
...
_CUBE_MESH: newton.Mesh | None = None
...
def _get_cube_mesh() -> newton.Mesh:
    ...
    if _CUBE_MESH is None:
        ...
        if BOX_REPRESENTATION is BoxRepresentation.MESH_SDF:
            _CUBE_MESH.build_sdf(max_resolution=SDF_RESOLUTION)
    return _CUBE_MESH
        ...
        for pyramid in range(num_pyramids):
            ...
            for level in range(pyramid_size):
                ...
                for i in range(num_cubes_in_row):
                    ...
                    if BOX_REPRESENTATION is BoxRepresentation.MESH_SDF:
                        builder.add_shape_mesh(body, mesh=_get_cube_mesh())
                    elif BOX_REPRESENTATION is BoxRepresentation.CONVEX_HULL:
                        builder.add_shape_convex_hull(body, mesh=_get_cube_mesh())
                    else:
