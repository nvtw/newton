...
###########################################################################
# Example Box Pyramid (Box3D Solver)
#
# Builds pyramids of box-shaped cubes with a wrecking ball on a ramp
# to stress-test narrow-phase contact generation using the Box3D TGS-Soft solver.
#
# Command: python -m newton.examples pyramid_box3d
#
###########################################################################
...
class Example:
    def __init__(self, viewer, args):
        ...
        self.sim_time = 0.0
        self.sim_substeps = 1  # Box3D handles substeps internally
        ...
        # Estimate max contacts based on number of boxes
        max_contacts = max(4096, box_count * 8)

        # Configure Box3D solver
        box3d_config = newton.solvers.Box3DConfig(
            num_substeps=args.num_substeps,
            num_velocity_iters=args.velocity_iters,
            num_relaxation_iters=args.relaxation_iters,
            contact_hertz=args.contact_hertz,
            max_bodies_per_world=max(1024, box_count + 10),
            max_contacts_per_world=max_contacts,
        )

        self.solver = newton.solvers.SolverBox3D(self.model, config=box3d_config)
    ...
    def simulate(self):
        """Run one frame of simulation (graph-capturable).

        Writes state_0 -> state_1, then copies state_1 -> state_0.
        This avoids Python object swaps which break CUDA graph replay.
        """
        ...
        # Copy result back to state_0 (CUDA graph compatible - same memory addresses)
    ...
    def create_parser():
        ...
        parser.add_argument(
            ...
            type=int,
            default=10,
            help="Number of solver substeps per frame.",
        )
        parser.add_argument(
            "--velocity-iters",
            type=int,
            default=1,
            help="Number of velocity (biased) iterations per substep.",
        )
        parser.add_argument(
            "--relaxation-iters",
            type=int,
            default=4,
            help="Number of relaxation iterations per substep.",
        )
        parser.add_argument(
            "--contact-hertz",
            type=float,
            default=30.0,
            help="Contact constraint spring frequency [Hz].",
        )
        return parser
