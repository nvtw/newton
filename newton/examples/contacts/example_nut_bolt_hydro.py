...
class Example:
    ...
    def test_final(self):
        ...
        # Check nuts rotated and moved down. The pre-callback hydroelastic
        # baseline rotates about 80 degrees over the example-test window, so a
        # 60 degree threshold catches stalled thread engagement without being
        # sensitive to small solver/contact-count variation.
        ...
        for i in range(len(self.nut_body_indices)):
            ...
            assert descent > min_descent, (
                f"Nut {i}: did not move down enough. "
                f"Descent={descent:.4f} (expected > {min_descent:.4f})"
            )
