...
states, contacts, and a solver for every trial; only solver layout, substeps,
position iterations, and the multi-world scheduler change. Material parameters,
joint mode, collision update frequency, and one velocity iteration remain fixed.
...
options other than the four tuned settings in that function. Set
...
checks the layout and one-axis work reductions. `default` runs 120 frames
with a 90-second budget and checks combined reductions and multi-world
schedulers. `thorough` runs 240 frames with a five-minute budget and explores
quarter budgets and substep/iteration tradeoffs. `--frames` and
