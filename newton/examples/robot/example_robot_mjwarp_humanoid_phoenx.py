...
###########################################################################
# Example Robot MuJoCo Warp Humanoid (PhoenX)
#
# Loads the humanoid benchmark MJCF from MuJoCo Warp and simulates it with
# SolverPhoenX.
#
# Command: python -m newton.examples robot_mjwarp_humanoid_phoenx
#
###########################################################################
...
from newton.examples.robot.mjwarp_benchmark_utils import benchmark_asset_dir, parse_prepare_refresh_stride
...
class Example:
    def __init__(self, viewer, args):
        ...
        asset_dir = benchmark_asset_dir("humanoid")

        humanoid = newton.ModelBuilder(up_axis=newton.Axis.Z)
        for i in range(args.humanoid_count):
            angle = math.radians(16.36 * i) if args.humanoid_count > 1 else 0.0
            xform = wp.transform(
                wp.vec3(4.0 * math.sin(angle), -4.0 * math.cos(angle), 0.0),
                wp.quat_from_axis_angle(wp.vec3(0.0, 0.0, 1.0), angle),
            )
            if args.humanoid_count == 1:
                xform = wp.transform_identity()
            humanoid.add_mjcf(
                str(asset_dir / "humanoid.xml"),
                xform=xform,
                ignore_names=("floor",),
        ...
        if self.world_count > 1:
            self.viewer.set_world_offsets((2.5, 2.5, 0.0))
    ...
    def create_parser():
        ...
        parser.add_argument(
            ...
            type=int,
            default=1,
            help="Number of humanoids per world. Use 3 for the MuJoCo Warp three-humanoids benchmark variant.",
        )
        parser.add_argument(
        ...
        parser.add_argument(
            ...
            action=argparse.BooleanOptionalAction,
            default=False,
            help="Enable self-collision while importing the humanoid.",
        )
        return parser
