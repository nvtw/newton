...
- Add on-disk caching of cooked texture-based SDFs via the new `cache_dir` argument on `SDF.create_from_mesh` and `Mesh.build_sdf`. Cached entries are content-addressed by mesh and build parameters, written atomically as an uncompressed `.npz` plus a JSON sidecar manifest, and versioned via `CACHE_FORMAT_VERSION` so format changes invalidate stale caches transparently
