...
- Add `prepare_refresh_stride` to `SolverPhoenX` for opt-in reuse of cached rigid-contact prepare data every second substep in contact-only `step_layout="single_world"` scenes without mass splitting or sleeping; leave the default `1` to rebuild prepare data every substep.
