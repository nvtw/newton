...
- Add `SolverMuJoCo(force_space_contact_gains=True)` and `SolverMuJoCo.SolrefMode` for configuring MuJoCo shape contact `solref` modes without importing internal constants.
...
- Fix `SolverMuJoCo` per-contact stiffness handling so hydroelastic contacts preserve authored stiffness and damping instead of being softened by MuJoCo timestep safety clamping.
