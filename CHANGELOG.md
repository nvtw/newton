...
- Filter kinematic-kinematic and static-kinematic contacts by default during broad-phase collision detection. `Model.shape_contact_pairs` still retains these pairs so direct consumers such as `SolverKamino` and hydroelastic SDF setup can opt into them; static-static pairs are always skipped. Set `CollisionPipeline(include_static_kinematic_pairs=True)` when kinematic-kinematic or static-kinematic contacts are required.
