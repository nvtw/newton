...
- Add native support for mirrored (negative-component) scale on mesh, convex hull, and SDF shapes in the collision pipeline; a single `Mesh` instance can now be shared across shapes with different signed scales without re-baking, with triangle winding, AABB conversion, SDF gradient remapping, and soft-contact margin heuristics all corrected for `det(scale) < 0` at kernel time
...
- Normalize negative scale components to their absolute value at `ModelBuilder.add_shape*` time for symmetric primitive shapes (sphere, box, capsule, cylinder, ellipsoid, plane, gaussian); these shapes are point-symmetric and produce identical geometry under sign flip
- Reject negative scale components on cone shapes at `ModelBuilder.add_shape_cone()` (would flip the apex/base or invalidate the slope test) and on heightfield shapes (mirroring is not yet supported)
