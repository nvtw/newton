...
- Add JP-MIS + greedy graph coloring to PhoenX (`SolverPhoenX`) via `IncrementalContactPartitioner.build_csr_greedy()`, default-on through `solver_config.PHOENX_USE_GREEDY_COLORING`. Each MIS commit picks the smallest color not used by its already-colored neighbors instead of the round number, dropping color counts to (or near) the `max_body_degree` lower bound — Kapla Tower goes from 78 to 28 colors (3.04x → 1.00x optimum) at comparable end-to-end wall time. Capped at 64 colors via a single-`int64` forbidden mask; falls back to a descriptive error if exceeded.
