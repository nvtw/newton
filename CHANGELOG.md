...
- `BodyContainer` gains a new `body_com` field (per-body origin-to-COM offset in the body-local frame). The `SolverPhoenX` and `SolverJitter` contact kernels now subtract this offset when projecting body-origin-relative contact anchors (Newton's narrow-phase convention) onto the COM-relative lever arm the solvers integrate. Without this offset, asymmetric meshes (bunny, nut, etc.) rendered through `state.body_q` appeared to sink into the contact surface by `|body_com|` (~10 cm on the bunny asset) because the stored anchor was treated as COM-relative. Symmetric primitives (boxes, spheres) are unaffected because their `body_com` is zero. Direct callers of `init_phoenx_bodies_kernel` must append `bodies.body_com` to the `outputs=[...]` list
- `SolverPhoenX` and `SolverJitter` contact columns now cover an entire `(shape_a, shape_b)` shape pair with a variable `contact_count` instead of being capped at 6 slots per column. Dense mesh and SDF manifolds are iterated serially (Gauss-Seidel within the pair) in a single kernel call, which eliminates the multi-column split that previously sank dense manifolds (bunny-on-plane, SDF nut-on-bolt) into contacting surfaces. `ContactContainer` is now indexed by the contact's sorted-buffer index `k` and sizes to `rigid_contact_max`; the forward contact map is keyed on `k` alone (no slot field). Direct callers of `ingest_contacts` / `gather_contact_warmstart` / `stamp_forward_contact_map` need their wiring updated accordingly
