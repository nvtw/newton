...
- Add fast parity-based SDF construction path for watertight meshes in `SDF.create_from_mesh`, using `wp.mesh_query_point_sign_parity` instead of winding numbers; selected automatically when `Mesh.is_watertight` is true, or forced via the new `use_parity` argument
...
- Build mesh SDFs via the texture-based sparse path only; `SDF.create_from_mesh` no longer allocates NanoVDB sparse/coarse volumes
