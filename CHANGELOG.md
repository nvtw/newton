...
- Parse `NewtonSDFCollisionAPI` attributes from USD in `ModelBuilder.add_usd()`, including enable toggles (`newton:sdfEnabled`, `newton:hydroelasticEnabled`), absolute and fractional narrow band / margin, texture format, hydroelastic stiffness (`newton:hydroelasticStiffness`), and applied-API schema defaults. Hydroelastic configuration is folded into `NewtonSDFCollisionAPI` and opted into via `newton:hydroelasticEnabled` (default `false`).
