...
- Add `SolverPhoenX(use_beam_for_cable=True)` opt-in flag that routes Newton `JointType.CABLE` joints through PhoenX's newer `JOINT_MODE_BEAM` (positional 3+2+1 anchor split with PD-soft bend / twist) instead of `JOINT_MODE_CABLE` (angular log-map rows). BEAM avoids CABLE's log-map convergence degradation at high bend stiffness; default remains CABLE for backward compatibility. Adds an analytical BEAM validation suite (`test_beam_joint.py`) covering bend / twist stiffness and damping vs damped-harmonic-oscillator theory, gain conversion across `rest_length`, multi-segment cantilever Euler-Bernoulli static deflection, BEAM ↔ CABLE asymptotic equivalence at small angles, and chain torque transmission, plus Newton-side adapter wiring tests (`test_beam_joint_newton.py`)
