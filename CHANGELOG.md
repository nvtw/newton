...
- Add an opt-in valid-kinematics import to experimental `SolverPhoenX.step()` so reduced-coordinate callers with coherent body and generalized state can skip redundant forward kinematics.
...
- Add explicit `collision_pipeline` injection and advanced mass-splitting scheduler controls to experimental `SolverPhoenX`, allowing applications to preserve custom broad-phase and contact-buffer sizing without deprecated `Model.contacts()` calls.
...
- Add D6 joint auto-dispatch to `SolverPhoenX`: `JointType.D6` configurations whose per-DoF lock pattern matches FIXED (all locked), BALL (3 linear locked + 3 angular free), REVOLUTE (3 linear locked + 2 angular locked + 1 angular free), or PRISMATIC (2 linear locked + 1 linear free + 3 angular locked), CYLINDRICAL, or PLANAR are now routed to the corresponding specialized constraint modes. Other generic bilateral layouts use precomputed complement rows in the direct mechanism system; finite generic-layout inequalities remain unsupported
- Add maximal `DISTANCE` joint bounds and scalar `joint_velocity_limit` enforcement for revolute and prismatic joints to `SolverPhoenX`, retaining these inequalities in PGS while direct equalities remain outside graph coloring.
- Add `JOINT_MODE_UNIVERSAL` (Hooke joint) to `SolverPhoenX`: D6 configurations with 3 linear locked + 1 angular locked + 2 angular free dispatch to a mode composing the BALL_SOCKET 3-row positional lock with a 1-row angular lock about the user-specified axis. Both free angular DoFs support armature, passive damping, and active implicit drives in the direct system.
- Add `JOINT_MODE_CYLINDRICAL` to `SolverPhoenX`: D6 configurations with 2 linear locked + 1 linear free + 2 angular locked + 1 angular free, with the two free axes parallel, dispatch to a new mode. The constraint set is PRISMATIC's 4-row tangent block (anchor-1 + anchor-2 perpendicular locks) minus PRISMATIC's anchor-3 scalar lock that gates rotation about the slide axis; the Schur complement collapses (no anchor-3 row → no complement needed). Armature, passive damping, and active implicit drives on either free DoF use separate rows in the same direct system
- Add `JOINT_MODE_PLANAR` to `SolverPhoenX`: D6 configurations with 1 linear locked + 2 linear free + 2 angular locked + 1 angular free, with the locked-linear and free-angular axes parallel (= plane normal), dispatch to a new mode. Three mechanism-direct rows constrain anchor motion along the plane normal, including the correct anchor lever arms, and relative angular velocity perpendicular to the normal. Armature, passive damping, and active implicit drives on all three free DoFs use separate rows in the same direct system
...
- Add `ViewerGL` simulation/render overlap to the common example runner. Shared ported examples snapshot body and particle positions before launching a CUDA-graph-compatible physics step on a nonblocking stream.
...
- Reuse one contact set and ownership graph across the three physics substeps of each experimental PhoenX G1 policy frame, while reprojecting contact anchors from live poses each substep.
...
- Fix legacy PGS and experimental maximal-projector PhoenX revolute armature by adding motor-side stator inertia to the parent and gear-reflected rotor inertia to the child.
