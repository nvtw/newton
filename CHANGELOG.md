...
- Add opt-in `SolverPhoenX(articulation_coarse_mode="auto")` multilevel correction for bilateral articulation rows. It selects GPU-local path, rooted-tree, or sparse-interpolation general-graph Galerkin coarse spaces, supplements normal contact and constraint PGS, supports mixed joint row counts, and remains compatible with CUDA graph capture.
