...
- Add `SolverSTFLIP` with a CUDA-graph-capturable packed sparse MAC grid, temporally staggered PIC/FLIP/APIC transfers, free-surface pressure projection, and two-way particle-rigid contact coupling.
...
- Accelerate `SolverSTFLIP` pressure projection with Chebyshev iterations and reduce the default `pressure_iterations` from 80 to 30. Set `pressure_iterations` explicitly to preserve an application-specific iteration budget.
...
- Fix `SolverSTFLIP` losing fluid volume at closed boundaries and applying particle-count-dependent forces to coupled rigid bodies.
