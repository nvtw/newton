"""v9: Muon + ELU + 256 hidden + alive bonus + obs norm."""
import os, resource, threading
_stop = threading.Event()
def wd():
    while not _stop.is_set():
        if resource.getrusage(resource.RUSAGE_SELF).ru_maxrss // 1024 > 10000: os._exit(1)
        _stop.wait(2.0)
...
config = PPOConfig(
    ...
    total_timesteps=10_000_000, seed=42,
    ...
    env_name="ANYmal v9", best_return_init=-1e9, device="cuda:0",
    log_interval=20, checkpoint_fn=ckpt,
    format_return_fn=lambda a,b,l,s: f"Ret {a:8.1f} | Best {b:8.1f} | Ent {l[3]:7.4f} | SPS {s:>7,.0f}",
)
PPOTrainer(config, lambda d: AnymalEnv(N, d, 42)).train()
# Quick test
